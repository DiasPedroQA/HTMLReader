...
def print_sysinfo():
    ...
    info["loadavg"] = "{:.1f}%, {:.1f}%, {:.1f}%".format(*tuple(x / psutil.cpu_count() * 100 for x in psutil.getloadavg()))
...
if POSIX:
    ...
    def copyload_shared_lib(suffix=""):
        ...
        libs = [x.path for x in psutil.Process().memory_maps() if os.path.splitext(x.path)[1] == ext and exe in x.path.lower()]
    ...
    def copyload_shared_lib(suffix=""):
        ...
        libs = [
            ...
            if x.path.lower().endswith(ext) and "python" in os.path.basename(x.path).lower() and "wow64" not in x.path.lower()
        ]
