...
from psutil._common import bcat, cat, debug, isfile_strict, memoize, memoize_when_activated, parse_environ_block, supports_ipv6, wrap_numbers
...
class TestSpecialMethods(PsutilTestCase):
    ...
    def test_no_such_process__repr__(self):
        ...
        assert repr(psutil.NoSuchProcess(321, name="name", msg="msg")) == "psutil.NoSuchProcess(pid=321, name='name', msg='msg')"
    ...
    def test_zombie_process__repr__(self):
        assert repr(psutil.ZombieProcess(321)) == 'psutil.ZombieProcess(pid=321, msg="PID still ' "exists but it's a zombie\")"
        assert (
            repr(psutil.ZombieProcess(321, name="name", ppid=320, msg="foo")) == "psutil.ZombieProcess(pid=321, ppid=320, name='name'," " msg='foo')"
        )

    def test_zombie_process__str__(self):
        assert str(psutil.ZombieProcess(321)) == "PID still exists but it's a zombie (pid=321)"
        assert str(psutil.ZombieProcess(321, name="name", ppid=320, msg="foo")) == "foo (pid=321, ppid=320, name='name')"

    def test_access_denied__repr__(self):
        assert repr(psutil.AccessDenied(321)) == "psutil.AccessDenied(pid=321)"
        assert repr(psutil.AccessDenied(321, name="name", msg="msg")) == "psutil.AccessDenied(pid=321, name='name', msg='msg')"
    ...
    def test_timeout_expired__repr__(self):
        ...
        assert (
            repr(psutil.TimeoutExpired(5, pid=321, name="name")) == "psutil.TimeoutExpired(pid=321, name='name', seconds=5, "
