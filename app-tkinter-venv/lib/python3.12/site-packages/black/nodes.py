...
def whitespace(leaf: Leaf, *, complex_subscript: bool, mode: Mode) -> str:  # noqa: C901
    ...
    if not prev:
        ...
        elif prevp.type == token.STAR and parent_type(prevp) == syms.star_expr and parent_type(prevp.parent) in (syms.subscriptlist, syms.tname_star):
...
def is_simple_decorator_trailer(node: LN, last: bool = False) -> bool:
    ...
    return node.type == syms.trailer and (
        ...
        or (last and len(node.children) == 2 and node.children[0].type == token.LPAR and node.children[1].type == token.RPAR)
...
def is_with_or_async_with_stmt(leaf: Leaf) -> bool:
    ...
    return bool(leaf.type == token.NAME and leaf.value == "with" and leaf.parent and leaf.parent.type == syms.with_stmt) or bool(
