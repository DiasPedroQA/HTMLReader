    ...
    def __call__(self, line: Line, _features: Collection[Feature], _mode: Mode) -> Iterator[Line]:
        ...
        if isinstance(match_result, Err):
            ...
            raise CannotTransform(
                f"The string transformer {self.__class__.__name__} does not recognize" " this line as one that it can transform."
            ) from cant_transform
...
class StringMerger(StringTransformer, CustomSplitMapMixin):
    ...
    def _merge_one_string_group(self, LL: list[Leaf], string_idx: int, is_valid_index: Callable[[int], bool]) -> tuple[int, Leaf]:
        ...
        def make_naked(string: str, string_prefix: str) -> str:
            ...
            if "f" in string_prefix:
                f_expressions = [string[span[0] + 1 : span[1] - 1] for span in iter_fexpr_spans(string)]  # +-1 to get rid of curly braces
    ...
    def _validate_msg(line: Line, string_idx: int) -> TResult[None]:
        ...
        for leaf in line.leaves[string_idx:]:
            ...
            if "f" in prefix and leaf.value[-1] != QUOTE and ("'" in leaf.value[len(prefix) + 1 : -1] or '"' in leaf.value[len(prefix) + 1 : -1]):
    ...
    def do_match(self, line: Line) -> TMatchResult:
        ...
        while True:
            ...
            if is_valid_index(idx - 2) and (
                LL[idx - 2].type == token.COLON or LL[idx - 2].type == token.NAME or LL[idx - 2].type in CLOSING_BRACKETS
            ):
                continue
...
class BaseStringSplitter(StringTransformer):
    ...
    def do_match(self, line: Line) -> TMatchResult:
        ...
        string_indices = match_result.ok()
        assert len(string_indices) == 1, f"{self.__class__.__name__} should only find one match at a time, found" f" {len(string_indices)}"
