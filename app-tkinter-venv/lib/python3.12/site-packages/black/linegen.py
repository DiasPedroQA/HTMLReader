...
def normalize_invisible_parens(node: Node, parens_after: set[str], *, mode: Mode, features: Collection[Feature]) -> None:  # noqa: C901
    ...
    if node.children[0].type == token.AWAIT and len(node.children) > 1:
        if node.children[1].type == syms.atom and node.children[1].children[0].type == token.LPAR:
            ...
            if isinstance(bracket_contents, Node) and (
                ...
                or any(isinstance(child, Leaf) and child.type == token.DOUBLESTAR for child in bracket_contents.children)
            ):
