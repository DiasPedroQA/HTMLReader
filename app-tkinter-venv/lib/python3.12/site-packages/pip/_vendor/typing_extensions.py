    ...
    class _SpecialGenericAlias(_special_generic_alias_base, _root=True):
        ...
        def __getitem__(self, params):
            ...
            if actual_len != self._nparams:
                ...
                raise TypeError(
                    f"Too {'many' if actual_len > self._nparams else 'few'}" f" arguments for {self};" f" actual {actual_len}, expected {expected}"
                )
        ...
        def __subclasscheck__(cls, other):
            ...
            if getattr(cls, "_is_protocol", False) and not _allow_reckless_class_checks():
                ...
                if (
                    ...
                    raise TypeError(
                        "Protocols with non-method members don't support issubclass()." f" Non-method members: {str(non_method_attrs)[1:-1]}."
                    )
...
if not hasattr(typing, "Concatenate"):
    ...
    class _ConcatenateGenericAlias(list):
        ...
        def __getitem__(self, args):
            ...
            plen = len(params)
            if alen != plen:
                raise TypeError(f"Too {'many' if alen > plen else 'few'} arguments for {self};" f" actual {alen}, expected {plen}")
            ...
            for arg in self.__args__:
                ...
                if isinstance(arg, TypeVar):
                    ...
                    if (isinstance(arg, typing._GenericAlias) and _is_unpack(arg)) or (
                        hasattr(_types, "GenericAlias") and isinstance(arg, _types.GenericAlias) and getattr(arg, "__unpacked__", False)
                    ):
                        raise TypeError(f"{arg} is not valid as type argument")

                elif isinstance(arg, typing._GenericAlias if not hasattr(_types, "GenericAlias") else (typing._GenericAlias, _types.GenericAlias)):
...
_PEP_649_OR_749_IMPLEMENTED = (
    hasattr(inspect, "get_annotations") and inspect.get_annotations.__kwdefaults__ is not None and "format" in inspect.get_annotations.__kwdefaults__
)
...
if _PEP_649_OR_749_IMPLEMENTED:
    ...
    def get_annotations(obj, *, globals=None, locals=None, eval_str=False, format=Format.VALUE):
        ...
        * STRING: return annotations as strings, in a format close to the original
        ...
        return_value = {key: value if not isinstance(value, str) else eval(value, globals, locals) for key, value in ann.items()}
        return return_value
        ...
        *type_params* is a tuple of type parameters that are in scope when
        ...
        if isinstance(type_, ForwardRef):
            ...
            return evaluate_forward_ref(
                type_, globals=globals, locals=locals, type_params=type_params, owner=owner, _recursive_guard=_recursive_guard, format=format
            )
