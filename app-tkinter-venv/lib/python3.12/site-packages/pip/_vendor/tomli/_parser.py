    ...
    def __init__(
        ...
        *args: Any,
        ...
        if args or not isinstance(msg, str) or not isinstance(doc, str) or not isinstance(pos, int):
            warnings.warn(
                "Free-form arguments for TOMLDecodeError are deprecated. " "Please set 'msg' (str), 'doc' (str) and 'pos' (int) arguments only.",
                DeprecationWarning,
                stacklevel=2,
            )
...
def parse_value(src: str, pos: Pos, parse_float: ParseFloat, nest_lvl: int) -> tuple[Pos, Any]:  # noqa: C901
    if nest_lvl > MAX_INLINE_NESTING:
        ...
        raise RecursionError("TOML inline arrays/tables are nested more than the allowed" f" {MAX_INLINE_NESTING} levels")  # pragma: no cover
