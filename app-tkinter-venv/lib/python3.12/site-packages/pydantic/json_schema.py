...
class GenerateJsonSchema:
        ...
        for key in core_schema_types:
            ...
            try:
                ...
                if os.getenv("PYDANTIC_PRIVATE_ALLOW_UNHANDLED_SCHEMA_TYPES"):
                    continue
                raise TypeError(
                    f"No method for generating JsonSchema for core_schema.type={key!r} " f"(expected: {type(self).__name__}.{method_name})"
                ) from e
        return mapping
    ...
    def typed_dict_schema(self, schema: core_schema.TypedDictSchema) -> JsonSchemaValue:
        ...
        named_required_fields: list[tuple[str, bool, CoreSchemaField]] = [
            (name, self.field_is_required(field, total), field) for name, field in schema["fields"].items() if self.field_is_present(field)
        ]
