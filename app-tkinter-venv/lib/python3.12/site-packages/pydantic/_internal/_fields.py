    ...
    for ann_name, (ann_type, evaluated) in type_hints.items():
                ...
                for b in bases:
                    if hasattr(b, ann_name):
                        if not (issubclass(b, BaseModel) and ann_name in getattr(b, "__pydantic_fields__", {})):
                            raise NameError(
                                f'Field "{ann_name}" conflicts with member {getattr(b, ann_name)}' f' of protected namespace "{protected_namespace}".'
                            )
                else:
                    valid_namespaces = ()
        ...
        for base in bases:
            ...
            if hasattr(base, ann_name):
                ...
                warnings.warn(
                    f'Field name "{ann_name}" in "{cls.__qualname__}" shadows an attribute in parent ' f'"{base.__qualname__}"',
                    UserWarning,
                )
        ...
        if assigned_value is PydanticUndefined:  # no assignment, just a plain annotation
            ...
            original_assignment = assigned_value._copy() if not evaluated and isinstance(assigned_value, FieldInfo_) else assigned_value
