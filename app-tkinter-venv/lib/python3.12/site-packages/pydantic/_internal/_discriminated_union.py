    ...
    * Handling various edge cases such as 'definitions', 'default', 'nullable' schemas, and more
    ...
    def _is_discriminator_shared(self, choice: core_schema.TaggedUnionSchema) -> bool:
        ...
        return inner_discriminator == self.discriminator or (
            isinstance(inner_discriminator, list) and (self.discriminator in inner_discriminator or [self.discriminator] in inner_discriminator)
        )

    def _infer_discriminator_values_for_choice(self, choice: core_schema.CoreSchema, source_name: str | None) -> list[str | int]:  # noqa C901
    ...
    def _infer_discriminator_values_for_field(self, field: CoreSchemaField, source: str) -> list[str | int]:
        ...
        if self._discriminator_alias is None:
            ...
            raise PydanticUserError(
                f"Aliases for discriminator {self.discriminator!r} must be the same " f"(got {alias}, {self._discriminator_alias})",
    ...
    def _infer_discriminator_values_for_inner_schema(self, schema: core_schema.CoreSchema, source: str) -> list[str | int]:
        ...
        if schema["type"] == "literal":
            ...
            raise PydanticUserError(
                f"Cannot use a mode={validator_type} validator in the" f" discriminator field {self.discriminator!r} of {source}",
        ...
        for discriminator_value in values:
            if discriminator_value in self._tagged_union_choices:
                ...
                if existing_choice != choice:
                    raise TypeError(f"Value {discriminator_value!r} for discriminator " f"{self.discriminator!r} mapped to multiple choices")
            else:
