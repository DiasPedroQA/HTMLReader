...
class CSSParser:
    ...
    css_tokens = (
        ...
        SpecialPseudoPattern(
            (
                ("pseudo_contains", (":contains", ":-soup-contains", ":-soup-contains-own"), PAT_PSEUDO_CONTAINS, SelectorPattern),
    ...
    def parse_pseudo_class(
        ...
        if complex_pseudo and pseudo in PSEUDO_COMPLEX:
            ...
            if pseudo == ":root":
                ...
                sel.nth.extend(
                    [ct.SelectorNth(1, False, 0, False, False, ct.SelectorList()), ct.SelectorNth(1, False, 0, False, True, ct.SelectorList())]
                )
            elif pseudo == ":only-of-type":
                sel.nth.extend(
                    [ct.SelectorNth(1, False, 0, True, False, ct.SelectorList()), ct.SelectorNth(1, False, 0, True, True, ct.SelectorList())]
                )
            has_selector = True
    ...
    def parse_pseudo_contains(self, sel: _Selector, m: Match[str], has_selector: bool) -> bool:
        ...
        pseudo = util.lower(css_unescape(m.group("name")))
        if pseudo == ":contains":
            warnings.warn("The pseudo class ':contains' is deprecated, ':-soup-contains' should be used moving forward.", FutureWarning)  # noqa: B028
