...
RE_DATETIME = re.compile(r"^(?P<year>[0-9]{4,})-(?P<month>[0-9]{2})-(?P<day>[0-9]{2})T(?P<hour>[0-9]{2}):(?P<minutes>[0-9]{2})$")
...
class _DocumentNav:
    ...
    def is_iframe(self, el: bs4.Tag | None) -> bool:
        ...
        return bool(((el.name if self.is_xml_tree(el) else util.lower(el.name)) == "iframe") and self.is_html_tag(el))  # type: ignore[attr-defined]
    ...
    def get_tag_children(self, el: bs4.Tag | None, start: int | None = None, reverse: bool = False, no_iframe: bool = False) -> Iterator[bs4.Tag]:
        """Get tag children."""
    ...
    def get_descendants(self, el: bs4.Tag | None, tags: bool = False, no_iframe: bool = False) -> Iterator[bs4.element.PageElement]:
        """Get descendants."""
    ...
    def get_text(self, el: bs4.Tag, no_iframe: bool = False) -> str:
        ...
        return "".join([node for node in self.get_descendants(el, no_iframe=no_iframe) if self.is_content_string(node)])  # type: ignore[misc]

    def get_own_text(self, el: bs4.Tag, no_iframe: bool = False) -> list[str]:
        """Get Own Text."""

        return [node for node in self.get_contents(el, no_iframe=no_iframe) if self.is_content_string(node)]  # type: ignore[misc]
...
class CSSMatch(_DocumentNav):
    ...
    def __init__(self, selectors: ct.SelectorList, scope: bs4.Tag | None, namespaces: ct.Namespaces | None, flags: int) -> None:
        """Initialize."""
...
class SoupSieve(ct.Immutable):
    ...
    def __repr__(self) -> str:  # pragma: no cover
        ...
        return f"SoupSieve(pattern={self.pattern!r}, namespaces={self.namespaces!r}, " f"custom={self.custom!r}, flags={self.flags!r})"
