...
class Checker:
    ...
    def _in_postponed_annotation(self):
        return self._in_annotation == AnnotationState.STRING or (
            self._in_annotation == AnnotationState.BARE and (self.annotationsFutureEnabled or sys.version_info >= (3, 14))
        )
    ...
    # same for operators
    AND = OR = ADD = SUB = MULT = DIV = MOD = POW = LSHIFT = RSHIFT = BITOR = BITXOR = BITAND = FLOORDIV = INVERT = NOT = UADD = USUB = EQ = NOTEQ = (
        LT
    ) = LTE = GT = GTE = IS = ISNOT = IN = NOTIN = MATMULT = ignore
