...
def _objdump_lexer_tokens(asm_lexer):
    ...
    return {
        "root": [
            ...
            ("( *)(" + hex_re + r"+:)( *\t)([a-zA-Z].*?)$", bygroups(Whitespace, Name.Label, Whitespace, using(asm_lexer))),
            ...
            # instruction
            ("( *)(" + hex_re + r"+:)(\t)((?:" + hex_re + hex_re + " )+)$", bygroups(Whitespace, Name.Label, Whitespace, Number.Hex)),
            # Skipped a few bytes
            (r"\t\.\.\.$", Text),
            # Relocation line
            # (With offset)
            (
                ...
                bygroups(Whitespace, Name.Label, Whitespace, Name.Property, Whitespace, Name.Constant, Punctuation, Number.Hex),
            ),
            # (Without offset)
            (
                r"(\t\t\t)(" + hex_re + r"+:)( )([^\t]+)(\t)(.*?)$",
...
class LlvmMirBodyLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (words(("oeq", "one", "ogt", "oge", "olt", "ole", "ugt", "uge", "ult", "ule"), prefix=r"floatpred\(", suffix=r"\)"), Name.Builtin),
            # Physical registers
            (r"\$\w+", String.Single),
            # Assignment operator
            (r"=", Operator),
...
class NasmLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            include("whitespace"),
            (identifier + ":", Name.Label),
            (rf"({identifier})(\s+)(equ)", bygroups(Name.Constant, Whitespace, Keyword.Declaration), "instruction-args"),
        ...
        "whitespace": [(r"\n", Whitespace), (r"[ \t]+", Whitespace), (r";.*", Comment.Single), (r"#.*", Comment.Single)],
        "punctuation": [
            (r"[,{}():\[\]]+", Punctuation),
            (r"[&|^<>+*/%~-]+", Operator),
            (r"[$]+", Keyword.Constant),
...
class TasmLexer(RegexLexer):
    ...
    string = r'"(\\"|[^"\n])*"|' + r"'(\\'|[^'\n])*'|" + r"`(\\`|[^`\n])*`"
    declkw = r"(?:res|d)[bwdqt]|times"
    register = r"(r[0-9][0-5]?[bwd]|" r"[a-d][lh]|[er]?[a-d]x|[er]?[sb]p|[er]?[sd]i|[c-gs]s|st[0-7]|" r"mm[0-7]|cr[0-4]|dr[0-367]|tr[3-7])\b"
    ...
    tokens = {
        "root": [
            ...
            (identifier + ":", Name.Label),
            (directives, Keyword, "instruction-args"),
            (rf"({identifier})(\s+)({datatype})", bygroups(Name.Constant, Whitespace, Keyword.Declaration), "instruction-args"),
            (declkw, Keyword.Declaration, "instruction-args"),
            (identifier, Name.Function, "instruction-args"),
            (r"[\r\n]+", Whitespace),
        ],
        ...
        "whitespace": [(r"[\n\r]", Whitespace), (r"(\\)([\n\r])", bygroups(Text, Whitespace)), (r"[ \t]+", Whitespace), (r";.*", Comment.Single)],
        "punctuation": [
            (r"[,():\[\]]+", Punctuation),
            (r"[&|^<>+*=/%~-]+", Operator),
            (r"[$]+", Keyword.Constant),
...
class Dasm16Lexer(RegexLexer):
    ...
    tokens = {
        ...
        "instruction-args": [(r",", Punctuation), (r"\[", Punctuation, "deref"), include("arg"), include("instruction-line")],
