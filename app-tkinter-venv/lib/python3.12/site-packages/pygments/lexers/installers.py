...
class RPMSpecLexer(RegexLexer):
    ...
    tokens = {
        ...
        "basic": [
            ...
            (r"%(attr|defattr|dir|doc(?:dir)?|setup|config(?:ure)?|" r"make(?:install)|ghost|patch[0-9]+|find_lang|exclude|verify)", Keyword),
            include("interpol"),
            (r"'.*?'", String.Single),
...
class SourcesListLexer(RegexLexer):
    ...
    tokens = {
        "root": [(r"\s+", Whitespace), (r"#.*?$", Comment), (r"^(deb(?:-src)?)(\s+)", bygroups(Keyword, Whitespace), "distribution")],
        "distribution": [
            (r"#.*?$", Comment, "#pop"),
        ...
        "escaped-distribution": [(r"\]", String.Other, "#pop"), (r"\$\(ARCH\)", Name.Variable), (r"[^\]$]+", String.Other), (r"\$", String.Other)],
        "components": [
            (r"#.*?$", Comment, "#pop:2"),
            (r"$", Text, "#pop:2"),
            (r"\s+", Whitespace),
...
class DebianControlLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"^((?:Build-|Pre-)?Depends(?:-Indep|-Arch)?)(:)(\s*)", bygroups(Keyword, Punctuation, Whitespace), "package_list"),
            (
