...
class BashLexer(RegexLexer):
    ...
    tokens = {
        ...
        "basic": [
            (
                r"\b(if|fi|else|while|in|do|done|for|then|return|function|case|" r"select|break|continue|until|esac|elif)(\s*)\b",
...
class BatchLexer(RegexLexer):
    ...
    def _make_begin_state(
        ...
        if compound:
            ...
            suffix = "/compound"
        return [
            ((r"\)", Punctuation, "#pop") if compound else (rf"\)((?=\()|{_token_terminator}){rest_of_line}", Comment.Single)),
            ...
            (rf"(goto{_keyword_terminator})({_space}?)(:?)", bygroups(Keyword, using(this, state="text"), Punctuation), f"label{suffix}"),
            (
                rf"(if(?:(?=\()|{_token_terminator})(?!\^))({_space}?)((?:/i{_token_terminator})?)({_space}?)((?:not{_token_terminator})?)({_space}?)",
                bygroups(Keyword, using(this, state="text"), Keyword, using(this, state="text"), Keyword, using(this, state="text")),
                ("(?", "if"),
            ),
            (
            ...
            (rf"(set{_keyword_terminator}){set_space}(/a)", bygroups(Keyword, using(this, state="text"), Keyword), f"arithmetic{suffix}"),
            (
                r'(set{}){}((?:/p)?){}((?:(?:(?:\^[{}]?)?[^"{}{}^={}]|'
                r'\^[{}]?[^"=])+)?)((?:(?:\^[{}]?)?=)?)'.format(
    ...
    def _make_call_state(compound, _label=_label, _label_compound=_label_compound):
        ...
        if compound:
            state.append((r"(?=\))", Text, "#pop"))
        state.append((r"(:?)(%s)" % (_label_compound if compound else _label), bygroups(Punctuation, Name.Label), "#pop"))
        return state
    ...
    def _make_redirect_state(
        ...
        return [
            (rf"((?:(?<=[{_nlws}])\d)?)(>>?&|<&)([{_nlws}]*)(\d)", bygroups(Number.Integer, Punctuation, Text, Number.Integer)),
            (
    ...
    tokens = {
        ...
        "text": [(r'"', String.Double, "string"), include("variable-or-escape"), (rf'[^"%^{_nlws}{_punct}\d)]+|.', Text)],
        "variable": [(r'"', String.Double, "string"), include("variable-or-escape"), (rf'[^"%^{_nl}]+|.', Name.Variable)],
        "for": [
            (rf"({_space})(in)({_space})(\()", bygroups(using(this, state="text"), Keyword, using(this, state="text"), Punctuation), "#pop"),
            include("follow"),
        ],
        "for2": [
            (r"\)", Punctuation),
        ...
        "for/f": [
            (rf'(")((?:{_variable}|[^"])*?")([{_nlws}]*)(\))', bygroups(String.Double, using(this, state="string"), Text, Punctuation)),
            (r'"', String.Double, ("#pop", "for2", "string")),
            (rf"('(?:%%|{_variable}|[\w\W])*?')([{_nlws}]*)(\))", bygroups(using(this, state="sqstring"), Text, Punctuation)),
            (rf"(`(?:%%|{_variable}|[\w\W])*?`)([{_nlws}]*)(\))", bygroups(using(this, state="bqstring"), Text, Punctuation)),
            include("for2"),
        ],
        ...
        "if2": [
            (rf"({_space}?)(==)({_space}?{_stoken})", bygroups(using(this, state="text"), Operator, using(this, state="text")), "#pop"),
            (rf"({_space})({_opword})({_space}{_stoken})", bygroups(using(this, state="text"), Operator.Word, using(this, state="text")), "#pop"),
        ],
        "(?": [(_space, using(this, state="text")), (r"\(", Punctuation, ("#pop", "else?", "root/compound")), default("#pop")],
...
class TcshLexer(RegexLexer):
    ...
    tokens = {
        ...
        "basic": [
            (r"\b(if|endif|else|while|then|foreach|case|default|" r"break|continue|goto|breaksw|end|switch|endsw)\s*\b", Keyword),
            (
...
class ExeclineLexer(RegexLexer):
    ...
    tokens = {
        ...
        "data": [(r'(?s)"(\\.|[^"\\$])*"', String.Double), (r'"', String.Double, "string"), (r"\s+", Text), (r'[^\s{}$"\\]+', Text)],
        "string": [
            (r'"', String.Double, "#pop"),
            (r'(?s)(\\\\|\\.|[^"\\$])+', String.Double),
            include("interp"),
