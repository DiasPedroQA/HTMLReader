...
from pygments.lexers import ClassNotFound, _googlesql_builtins, _mysql_builtins, _postgres_builtins, _sql_builtins, _tsql_builtins, get_lexer_by_name
from pygments.token import Comment, Generic, Keyword, Literal, Name, Number, Operator, Punctuation, String, Text, Whitespace
...
class PostgresLexer(PostgresBase, RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"(" + "|".join(s.replace(" ", r"\s+") for s in _postgres_builtins.DATATYPES + _postgres_builtins.PSEUDO_TYPES) + r")\b", Name.Builtin),
...
class PlPgsqlLexer(PostgresBase, RegexLexer):
    ...
    for i, pattern in enumerate(tokens["root"]):
        if pattern[1] == Keyword:
            tokens["root"][i] = (words(_postgres_builtins.KEYWORDS + _postgres_builtins.PLPGSQL_KEYWORDS, suffix=r"\b"), Keyword)
            del i
            break
    else:
...
class PostgresExplainLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (r"(Sort Key|Group Key|Presorted Key|Hash Key)(:)( )", bygroups(Comment.Preproc, Punctuation, Whitespace), "object_name"),
            ...
            (
                words(
                    ("Rows Removed by Join Filter", "Rows Removed by Filter", "Rows Removed by Index Recheck", "Heap Fetches", "never executed"),
                    suffix=r"\b",
                ),
                Name.Exception,
            ),
...
class MySqlLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (words(_mysql_builtins.MYSQL_FUNCTIONS, prefix=r"\b", suffix=r"\b(\s*)(\()"), bygroups(Name.Function, Whitespace, Punctuation)),
            # Schema object names
            #
...
class GoogleSqlLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (words(_googlesql_builtins.functionnames, prefix=r"\b", suffix=r"\b(\s*)(\()"), bygroups(Name.Function, Whitespace, Punctuation)),
