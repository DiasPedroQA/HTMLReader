...
class Perl6Lexer(ExtendedRegexLexer):
    ...
    tokens = {
        "common": [
            ...
            (r"(regex|token|rule)(?!" + PERL6_IDENTIFIER_RANGE + r")(\s*" + PERL6_IDENTIFIER_RANGE + "+)?", bygroups(Keyword, Name), "pre-token"),
            ...
            (r"\$(?:<.*?>)+", Name.Variable),
            (
                r"(?:q|qq|Q)[a-zA-Z]?\s*(?P<adverbs>:[\w\s:]+)?\s*(?P<delimiter>(?P<first_char>[^0-9a-zA-Z:\s])" r"(?P=first_char)*)",
            ...
            (r"(?<=[=(,])\s*/(?:\\\\|\\/|.)*?/", String.Regex),
            (r"m\w+(?=\()", Name),
            (r"(?:m|ms|rx)\s*(?P<adverbs>:[\w\s:]+)?\s*(?P<delimiter>(?P<first_char>[^\w:\s])" r"(?P=first_char)*)", brackets_callback(String.Regex)),
