...
class LdaprcLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            (r"#.*", Comment.Single),
            (r"\s+", Whitespace),
            (rf"({_boolean_keywords})(\s+)(on|true|yes|off|false|no)$", bygroups(Keyword, Whitespace, Keyword.Constant)),
            ...
            (rf"(SASL_SECPROPS)(\s+)((?:{_secprops})(?:,{_secprops})*)", bygroups(Keyword, Whitespace, Keyword.Constant)),
            (r"(SASL_CBINDING)(\s+)(none|tls-unique|tls-endpoint)", bygroups(Keyword, Whitespace, Keyword.Constant)),
            (r"(TLS_REQ(?:CERT|SAN))(\s+)(allow|demand|hard|never|try)", bygroups(Keyword, Whitespace, Keyword.Constant)),
