...
class TurtleLexer(RegexLexer):
    ...
    patterns = {"PNAME_NS": r"((?:[a-zA-Z][\w-]*)?\:)", "IRIREF": r'(<[^<>"{}|^`\\\x00-\x20]*>)'}  # Simplified character range

    tokens = {
        "root": [
            (r"\s+", Text),
            # Base / prefix
            (r"(@base|BASE)(\s+){IRIREF}(\s*)(\.?)".format(**patterns), bygroups(Keyword, Whitespace, Name.Variable, Whitespace, Punctuation)),
            (
            ...
            (r"(" + PN_PREFIX + r")?(\:)(" + PN_LOCAL + r")?", bygroups(Name.Namespace, Punctuation, Name.Tag)),
            # BlankNodeLabel
            (r"(_)(:)([" + PN_CHARS_U_GRP + r"0-9]([" + PN_CHARS_GRP + r".]*" + PN_CHARS + ")?)", bygroups(Name.Namespace, Punctuation, Name.Tag)),
            # Comment
            (r"#[^\n]+", Comment),
