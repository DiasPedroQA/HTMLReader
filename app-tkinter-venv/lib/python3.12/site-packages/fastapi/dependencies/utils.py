    ...
    *,
    ...
    if isinstance(value, params.Depends):
        ...
        assert field_info is None, (
            "Cannot specify a FastAPI annotation in `Annotated` and `Depends` as a" f" default value together for {param_name!r}"
        )
        depends = value
    # Get FieldInfo from default value
    elif isinstance(value, FieldInfo):
        assert field_info is None, "Cannot specify FastAPI annotations in `Annotated` and default value" f" together for {param_name!r}"
        field_info = value
        if PYDANTIC_V2:
    ...
    if field_info is not None:
        ...
        if is_path_param:
            assert isinstance(field_info, params.Path), f"Cannot use `{field_info.__class__.__name__}` for path param" f" {param_name!r}"
