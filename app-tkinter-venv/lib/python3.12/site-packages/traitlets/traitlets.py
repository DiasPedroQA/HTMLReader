...
class TraitType(BaseDescriptor, t.Generic[G, S]):
    ...
    def get(self, obj: HasTraits, cls: type[t.Any] | None = None) -> G | None:
        ...
        try:
            ...
            if default is Undefined:
                warn(
                    "Explicit using of Undefined as the default value " "is deprecated in traitlets 5.0, and may cause " "exceptions in the future.",
                    DeprecationWarning,
                    stacklevel=2,
                )
    ...
    def tag(self, **metadata: t.Any) -> Self:
        ...
        if maybe_constructor_keywords:
            warn(
                "The following attributes are set in using `tag`, but seem to be constructor keywords arguments: %s " % maybe_constructor_keywords,
                UserWarning,
                stacklevel=2,
            )
        ...
        for name in dir(cls):
            ...
            if isinstance(value, TraitType):
                ...
                for c in mro_trait:
                    ...
                    elif isinstance(trait, Instance) and trait.default_args is None and trait.default_kwargs is None and trait.allow_none:
                        cls._static_immutable_initial_values[name] = None
    ...
    def validate(self, obj: t.Any, value: t.Any) -> G:
        ...
        if isinstance(value, str):
            try:
                ...
                raise TraitError(f"The '{self.name}' trait of {obj} instance must be a type, but " f"{value!r} could not be imported") from e
        try:
        ...
        if len(s) >= 3:
            ...
            for quote in ('"', "'"):
                if s[:2] == f"b{quote}" and s[-1] == quote:
                    ...
                    s = s[2:-1]
                    warn(
                        "Supporting extra quotes around Bytes is deprecated in traitlets 5.0. " f"Use {s!r} instead of {old_s!r}.",
                        DeprecationWarning,
                        stacklevel=2,
                    )
                    break
    ...
    def from_string_list(self, s_list: list[str]) -> T | None:
        ...
        if len(s_list) == 1:
            ...
            if r == "None" and self.allow_none:
                return None
            if len(r) >= 2 and any(r.startswith(start) and r.endswith(end) for start, end in self._literal_from_string_pairs):
                if self.this_class:
                ...
                warn(
                    ...
                    "You can pass `--{0} item` ... multiple times to add items to a list.".format(clsname + self.name, r),
                    DeprecationWarning,
                    stacklevel=2,
                )
...
class TCPAddress(TraitType[G, S]):
    ...
    if t.TYPE_CHECKING:
        ...
        def __init__(
            self: TCPAddress[tuple[str, int] | None, tuple[str, int] | None] | TCPAddress[tuple[str, int], tuple[str, int]],
