    ...
    def __setitem__(self, key: str, value: t.Any) -> None:
        if _is_section_key(key):
            if not isinstance(value, Config):
                raise ValueError("values whose keys begin with an uppercase " f"char must be Config instances: {key!r}, {value!r}")
            ...
            for key in keys:
                ...
                if traitname in argparse_traits:
                    ...
                    if "action" in argparse_kwds and traitname in alias_flags:
                        ...
                        # on the same name
                        raise ArgumentError(f"The alias `{key}` for the 'append' sequence " f"config-trait `{traitname}` cannot be also a flag!'")
