...
import_re = re.compile(r"(?P<name>[^\W\d]\w*?)" r"(?P<package>[/\\]__init__)?" r"(?P<suffix>%s)$" % r"|".join(re.escape(s) for s in _suffixes))
...
def magic_run_completer(self, event):
    ...
    if any(magic_run_re.match(c) for c in comps):
        ...
        pys = [
            f.replace("\\", "/") for f in lglob(relpath + "*.py") + lglob(relpath + "*.ipy") + lglob(relpath + "*.ipynb") + lglob(relpath + "*.pyw")
        ]
