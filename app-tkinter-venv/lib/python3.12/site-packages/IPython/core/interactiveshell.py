...
class InteractiveShell(SingletonConfigurable):
    ...
    banner1 = Unicode(default_banner, help="""The part of the banner to be printed before the profile""").tag(config=True)
    ...
    input_transformers_post: List = List([], help="A list of string input transformers, to be applied after IPython's " "own input transformations.")
    ...
    colors = Unicode("neutral", help="Set the color scheme (nocolor, neutral, linux, lightbg).").tag(config=True)
    ...
    def _check_colors(self, proposal):
        ...
        if not new == new.lower():
            warn(
                f"`TerminalInteractiveShell.colors` is now lowercase: `{new.lower()}`," " non lowercase, may be invalid in the future.",
                DeprecationWarning,
                stacklevel=2,
            )
        return new.lower()
    ...
    def system_raw(self, cmd):
        ...
        if main_cmd in has_magic_alternatives:
            warnings.warn(
                ("You executed the system command !{0} which may not work " "as expected. Try the IPython magic %{0} instead.").format(main_cmd)
            )
        ...
        *,
        ...
        def error_before_exec(value):
            if store_history:
                if self.history_manager:
                    ...
                    self.history_manager.exceptions[self.execution_count] = self._format_exception_for_storage(value)
                self.execution_count += 1
        ...
        if store_history:
            ...
            if result.error_in_exec:
                # Store formatted traceback and error details
                self.history_manager.exceptions[exec_count] = self._format_exception_for_storage(result.error_in_exec)
            ...
            if contains_exceptiongroup(evalue):
                ...
                try:
                    ...
                    if hasattr(evalue, "_render_traceback_"):
                        ...
                        stb = self.InteractiveTB.structured_traceback(etype, evalue, tb, tb_offset=1)
                except Exception:
    ...
    def transform_ast(self, node):
        ...
        for transformer in self.ast_transformers:
            try:
                ...
                warn("AST transformer %r threw an error. It will be unregistered. %s" % (transformer, e))
