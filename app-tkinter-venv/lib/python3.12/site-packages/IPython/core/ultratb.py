...
class ListTB(TBTools):
    ...
    def structured_traceback(
        ...
        if etb is not None:
            ...
            if exception and (id(exception[1]) not in chained_exc_ids):
                chained_exception_message: list[str] = self.prepare_chained_exception_message(evalue.__cause__)[0] if evalue is not None else [""]
    ...
    def _format_list(self, extracted_list: list[Any]) -> list[str]:
        ...
        for ind, (filename, lineno, name, line) in enumerate(extracted_list):
            ...
            item = theme_table[self._theme_name].format(
                [(Token.NormalEm if em else Token.Normal, "  ")] + _tokens_filename(em, filename, lineno=lineno)
            )
                    ...
                    if value.offset is not None:
                        ...
                        for c in textline[i : value.offset - 1]:
                            if c.isspace():
                                ...
                                s += " "
                        output_list.append(theme_table[self._theme_name].format([(Token.Caret, s + "^"), (Token, "\n")]))
    ...
    def show_exception_only(self, etype: BaseException | None, evalue: TracebackType | None) -> None:
        """Only print the exception type and message, without a traceback.
...
class VerboseTB(TBTools):
    ...
    def format_record(self, frame_info: FrameInfo) -> str:
        ...
        if func == "<module>":
            ...
            try:
                ...
                call = theme_table[self._theme_name].format([(Token, "in "), (Token.VName, func), (Token.ValEm, scope)])
            except KeyError:
        ...
        if frame_info._sd is None:
            ...
            result = theme_table[self._theme_name].format(_tokens_filename(True, frame_info.filename, lineno=frame_info.lineno))
            result += ", " if call else ""
            result += f"{call}\n"
    ...
    def format_exception(self, etype, evalue):
        ...
        # ... and format it
        return [
            theme_table[self._theme_name].format([(Token.ExcName, etype_str), (Token, ": "), (Token, evalue_str)]),
            *(theme_table[self._theme_name].format([(Token, _safe_string(py3compat.cast_unicode(n), "note"))]) for n in notes),
        ]
    ...
    def structured_traceback(
        ...
        formatted_exceptions: list[list[str]] = self.format_exception_as_a_whole(etype, evalue, etb, context, tb_offset)
    ...
    def structured_traceback(
        ...
        if isinstance(value, SyntaxError) and isinstance(value.filename, str) and isinstance(value.lineno, int):
            ...
            if newtext:
                value.text = newtext
        self.last_syntax_error = value
        return super(SyntaxTB, self).structured_traceback(etype, value, etb, tb_offset=tb_offset, context=context)
