...
class Completion:
    ...
    def __init__(
        ...
        *,
        ...
        warnings.warn(
            "``Completion`` is a provisional API (as of IPython 6.0). " "It may change without warnings. " "Use in corresponding context manager.",
...
def rectify_completions(text: str, completions: _IC, *, _debug: bool = False) -> _IC:
    ...
    """
    warnings.warn(
        "`rectify_completions` is a provisional API (as of IPython 6.0). " "It may change without warnings. " "Use in corresponding context manager.",
        category=ProvisionalCompleterWarning,
        stacklevel=2,
    )
...
class Completer(Configurable):
    ...
    use_jedi = Bool(
        default_value=JEDI_INSTALLED, help="Experimental: Use Jedi to generate autocompletions. " "Default to True if jedi is installed."
    ).tag(config=True)
    ...
    debug = Bool(
        default_value=False, help="Enable debug for the Completer. Mostly print extra " "information for experimental jedi integration."
    ).tag(config=True)
    ...
    def global_matches(self, text):
        ...
        for lst in [list(self.namespace.keys()), list(self.global_namespace.keys())]:
            shortened = {"_".join([sub[0] for sub in word.split("_")]): word for word in lst if snake_case_re.match(word)}
    ...
    def _attr_matches(self, text: str, include_prefix: bool = True) -> tuple[Sequence[str], str]:
...
def match_dict_keys(
    ...
    filtered_key_is_final: dict[Union[str, bytes, int, float], _DictKeyState] = defaultdict(lambda: _DictKeyState.BASELINE)
    ...
    for k in keys:
        ...
        if isinstance(k, tuple):
            if filter_prefix_tuple(k):
                ...
                filtered_key_is_final[key_fragment] |= _DictKeyState.END_OF_TUPLE if len(k) == prefix_tuple_size + 1 else _DictKeyState.IN_TUPLE
...
def _make_signature(completion) -> str:
    ...
    return "(%s)" % ", ".join(
        [f for f in (_formatparamchildren(p) for signature in completion.get_signatures() for p in signature.defined_names()) if f]
    )
...
class IPCompleter(Completer):
    ...
    def file_matcher(self, context: CompletionContext) -> SimpleMatcherResult:
        ...
        if text == "":
            return {
                "completions": [SimpleCompletion(text=text_prefix + protect_filename(f), type="path") for f in self.glob("*")],
                "suppress": False,
            }
        ...
        return {
            "completions": [SimpleCompletion(text=x + "/" if os.path.isdir(x) else x, type="path") for x in matches],
            "suppress": False,
        }
    ...
    def magic_color_matcher(self, context: CompletionContext) -> SimpleMatcherResult:
        ...
        if len(texts) == 2 and (texts[0] == "colors" or texts[0] == "%colors"):
            prefix = texts[1]
            return SimpleMatcherResult(
                completions=[SimpleCompletion(color, type="param") for color in theme_table.keys() if color.startswith(prefix)],
                suppress=False,
            )
        return SimpleMatcherResult(
            completions=[],
    ...
    def python_matcher(self, context: CompletionContext) -> SimpleMatcherResult:
        ...
        if completion_type == self._CompletionContextType.ATTRIBUTE:
            try:
                ...
                return _convert_matcher_v1_result_to_v2(matches, type="attribute", fragment=fragment)
            except NameError:
    ...
    def completions(self, text: str, offset: int) -> Iterator[Completion]:
        ...
        """
        warnings.warn(
            "_complete is a provisional API (as of IPython 6.0). " "It may change without warnings. " "Use in corresponding context manager.",
            category=ProvisionalCompleterWarning,
            stacklevel=2,
        )
    ...
    def complete(self, text=None, line_buffer=None, cursor_pos=None) -> tuple[str, Sequence[str]]:
        ...
        """
        warnings.warn(
            "`Completer.complete` is pending deprecation since " "IPython 6.0 and will be replaced by `Completer.completions`.",
        ...
        for matcher_id, matcher in matchers.items():
            ...
            try:
                if _is_matcher_v1(matcher):
                    result = _convert_matcher_v1_result_to_v2_no_no(matcher(text), type=_UNKNOWN_TYPE)
