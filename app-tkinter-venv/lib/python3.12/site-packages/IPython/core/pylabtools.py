...
def __getattr__(name):
    if name in ("backends", "backend2gui"):
        warnings.warn(
            f"{name} is deprecated since IPython 8.24, backends are managed " "in matplotlib and can be externally registered.",
            DeprecationWarning,
        )
...
def _list_matplotlib_backends_and_gui_loops() -> list[str]:
    ...
    if _matplotlib_manages_backends():
        ...
        ret = backend_registry.list_all() + [_convert_gui_from_matplotlib(gui) for gui in backend_registry.list_gui_frameworks()]
    else:
