...
if TYPE_CHECKING:
    TDefinitionAst = ast.FunctionDef | ast.AsyncFunctionDef | ast.Import | ast.ImportFrom | ast.Assign | ast.AnnAssign
...
class DeduperReloader(DeduperReloaderPatchingMixin):
    ...
    def update_sources(self) -> None:
        ...
        for new_modname in sys.modules.keys() - self.source_by_modname.keys():
            ...
            if (
                (fname := get_module_file_name(new_module)) is None  # type:ignore [redundant-expr]
    ...
    def is_enum_subclass(node: ast.Module | ast.ClassDef) -> bool:
        ...
        for base in node.bases:
            if isinstance(base, ast.Name) and base.id == "Enum":
                return True
            elif isinstance(base, ast.Attribute) and base.attr == "Enum" and isinstance(base.value, ast.Name) and base.value.id == "enum":
                return True
        return False
    ...
    @classmethod
    def _gather_children(cls, body: list[ast.stmt], parent_node: ast.Module | ast.ClassDef) -> GatherResult:
        """
        ...
        for ast_node in body:
            ...
            if isinstance(ast_elt, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                result.import_defs.append((tuple(name.asname or name.name for name in ast_elt.names), ast_elt))
    ...
    def detect_autoreload(
        ...
        old_defs_by_name: dict[str, ast.AST] = {name: ast_def for names, ast_def in old_result.all_defs() for name in names}
        new_defs_by_name: dict[str, ast.AST] = {name: ast_def for names, ast_def in new_result.all_defs() for name in names}
        ...
        for names, new_ast_def in new_result.all_defs():
            ...
            for name in names:
                if new_defs_by_name[name] is not new_ast_def:
                    continue
                if name not in old_defs_by_name or not compare_ast(new_ast_def, old_defs_by_name[name]):
        ...
        for name, new_ast_def_class in new_result.classes.items():
            ...
            elif not compare_ast(new_ast_def_class, old_result.classes[name]) and not self.detect_autoreload(
    ...
    def _patch_namespace_inner(self, ns: ModuleType | type, prefixes: list[str] | None = None) -> bool:
        """
        ...
        for names, new_ast_def in cur.defs_to_reload:
            ...
            if isinstance(new_ast_def, (ast.FunctionDef, ast.AsyncFunctionDef)) and (name := names[0]) in namespace_to_check.__dict__:
                assert len(names) == 1
                ...
                if isinstance(to_patch_to, property) and isinstance(to_patch_from, property):
                    for attr in ("fget", "fset", "fdel"):
                        if getattr(to_patch_to, attr) is None or getattr(to_patch_from, attr) is None:
                ...
                elif not isinstance(to_patch_to, property) and not isinstance(to_patch_from, property):
    ...
    def maybe_reload_module(self, module: ModuleType) -> bool:
        ...
        if old_source_code := self.source_by_modname.get(modname):
            ...
            with ctx:
                ...
                if self.detect_autoreload(old_module_ast, new_module_ast) and self._check_dependents() and self._patch_namespace(module):
                    patched_flag = True
    ...
    def _gather_dependents(self, body: list[ast.stmt], body_prefixes: list[str] | None = None) -> bool:
        ...
        for ast_node in body:
            ...
            for decorator in ast_elt.decorator_list:
                ...
                self.dependency_graph.setdefault(decorator_path_tuple, []).append(cur_dependency_node)
        return True
