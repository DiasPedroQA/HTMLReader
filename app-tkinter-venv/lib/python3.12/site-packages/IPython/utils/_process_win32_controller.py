...
class Win32ShellCommandController:
    ...
    def __enter__(self):
        ...
        try:
            ...
            if not CreateProcess(
                None, "cmd.exe /c " + cmd, None, None, True, dwCreationFlags, None, None, ctypes.byref(siStartInfo), ctypes.byref(piProcInfo)
            ):
                raise ctypes.WinError()
    ...
    def run(self, stdout_func=None, stdin_func=None, stderr_func=None):
        ...
        if stderr_func is not None and self.mergeout:
            raise RuntimeError(
                "Shell command was initiated with " "merged stdin/stdout, but a separate stderr_func " "was provided to the run() method"
            )
