...
class AppendAutoSuggestionInAnyLine(Processor):
    ...
    def apply_transformation(self, ti: TransformationInput) -> Transformation:
        ...
        noop = lambda text: Transformation(fragments=ti.fragments + [(self.style, " " + text if self._debug else "")])
        ...
        if is_last_line:
            ...
            if n_elided:
                ...
                return Transformation(
                    ti.get_line(last_line_number - len(suggestions) + 1) + ([(self.style, "shift-last-line")] if self._debug else [])
                )
        ...
        async for reply_and_chunks in self._llm_provider.stream_inline_completions(request):
            ...
            if isinstance(reply_and_chunks, jai_models.InlineCompletionReply):
                if len(reply_and_chunks.list.items) > 1:
                    raise ValueError("Terminal IPython cannot deal with multiple LLM suggestions at once")
