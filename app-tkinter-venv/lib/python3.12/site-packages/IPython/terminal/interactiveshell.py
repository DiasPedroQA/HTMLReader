...
from IPython.utils.terminal import restore_term_title, set_term_title, toggle_set_term_title
...
def _backward_compat_continuation_prompt_tokens(method, width: int, *, lineno: int, wrap_count: int):
    """
    ...
    if "lineno" in inspect.signature(method).parameters or any([p.kind == p.VAR_KEYWORD for p in sig.parameters.values()]):
        extra["lineno"] = lineno
    if "line_number" in inspect.signature(method).parameters or any([p.kind == p.VAR_KEYWORD for p in sig.parameters.values()]):
        extra["line_number"] = lineno

    if "wrap_count" in inspect.signature(method).parameters or any([p.kind == p.VAR_KEYWORD for p in sig.parameters.values()]):
...
class TerminalInteractiveShell(InteractiveShell):
    ...
    autoformatter = Unicode(None, help="Autoformatter to reformat Terminal code. Can be `'black'`, `'yapf'` or `None`", allow_none=True).tag(
        config=True
    )
    ...
    term_title_format = Unicode(
        "IPython: {cwd}", help="Customize the terminal title format.  This is a python format string. " + "Available substitutions are: {cwd}."
    ).tag(config=True)
    ...
    def _merge_shortcuts(self, user_shortcuts):
        ...
        allowed_commands = {create_identifier(binding.command): binding.command for binding in KEY_BINDINGS}
        allowed_commands.update({create_identifier(command): command for command in UNASSIGNED_ALLOWED_COMMANDS})
        shortcuts_to_skip = []
        shortcuts_to_add = []
        ...
        for shortcut in user_shortcuts:
            ...
            if modifying_existing:
                ...
                if len(matching) == 0:
                    ...
                    raise ValueError(f"Multiple shortcuts matching {specification} found," f" please add keys/filter to select one of: {matching}")
    ...
    def inputhook(self, context):
        warn(
            "inputkook seem unused, and marked for deprecation/Removal as of IPython 9.0. " "Please open an issue if you are using it.",
