...
def get_verbose_details(manager):
    ...
    bits.extend([tpl % (item, sum(score["SEVERITY"]), sum(score["CONFIDENCE"])) for (item, score) in zip(manager.files_list, manager.scores)])
...
def report(manager, fileobj, sev_level, conf_level, lines=-1):
    ...
    if not manager.quiet or manager.results_count(sev_level, conf_level):
        ...
        bits.append("\tTotal lines skipped (#nosec): %i" % (manager.metrics.data["_totals"]["nosec"]))
