...
"""
    ...
    skipped_str = "".join(f"{fname} <b>reason:</b> {reason}<br>" for fname, reason in manager.get_skipped())
    if skipped_str:
    ...
    for index, issue in enumerate(issues):
        if not baseline or len(issues[issue]) == 1:
            ...
            for candidate in issues[issue]:
                candidate_code = html_escape(candidate.get_code(lines, True).strip("\n").lstrip(" "))
    ...
    report_contents = report_block.format(metrics=metrics_summary, skipped=skipped_text, results=results_str)
