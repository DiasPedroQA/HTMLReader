...
def expect_token(token, tok_type, tok_str=None):
    ...
    if not match_token(token, tok_type, tok_str):
        raise ValueError(
            "Expected token %s, got %s on line %s col %s" % (token_repr(tok_type, tok_str), str(token), token.start[0], token.start[1] + 1)
        )
...
SINGLETONS = {
    c for n, c in ast.__dict__.items() if isinstance(c, type) and issubclass(c, (ast.expr_context, ast.boolop, ast.operator, ast.unaryop, ast.cmpop))
}
...
def is_slice(node):
    ...
    return node.__class__.__name__ in ("Slice", "ExtSlice") or (node.__class__.__name__ == "Tuple" and any(map(is_slice, cast(ast.Tuple, node).elts)))


def is_empty_astroid_slice(node):
    # type: (AstNode) -> bool
    return node.__class__.__name__ == "Slice" and not isinstance(node, ast.AST) and node.lower is node.upper is node.step is None
