    ...
    if args:
        ...
        if isinstance(argument_types, list) and not any(type(item) is ParamSpec for item in argument_types):
            ...
            unfulfilled_kwonlyargs = [
                param.name for param in signature.parameters.values() if param.kind == Parameter.KEYWORD_ONLY and param.default == Parameter.empty
            ]
            if unfulfilled_kwonlyargs:
                raise TypeCheckError(f"has mandatory keyword-only arguments in its declaration: " f'{", ".join(unfulfilled_kwonlyargs)}')
            ...
            if num_mandatory_pos_args > len(argument_types):
                ...
                raise TypeCheckError(
                    f"has too few arguments in its declaration; expected " f"{len(argument_types)} but {num_positional_args} argument(s) " f"declared"
                )
    ...
    try:
        for attrname, callable_args in expected_methods.items():
            try:
                ...
                if attrname in subject_annotations:
                    raise TypeCheckError(
                        f"is not compatible with the {origin_type.__qualname__} protocol " f"because its {attrname!r} attribute is not a method"
                    ) from None
                else:
                    raise TypeCheckError(
                        f"is not compatible with the {origin_type.__qualname__} protocol " f"because it has no method named {attrname!r}"
                    ) from None

            if not callable(method):
                raise TypeCheckError(
                    f"is not compatible with the {origin_type.__qualname__} protocol " f"because its {attrname!r} attribute is not a callable"
                )
        ...
        for attrname in expected_noncallable_members:
            ...
            if attrname not in subject_annotations and not hasattr(subject, attrname):
                raise TypeCheckError(
                    f"is not compatible with the {origin_type.__qualname__} protocol " f"because it has no attribute named {attrname!r}"
                )
    except TypeCheckError as exc:
...
def check_type_internal(
    ...
    if isclass(origin_type):
        ...
        warnings.warn(
            f"Skipping type check against {origin_type!r}; this looks like a " f"string-form forward reference imported from another module",
            TypeHintWarning,
            stacklevel=get_stacklevel(),
        )
