        ...
        if node.slice:
            ...
            if isinstance(slice_value, Tuple):
                ...
                if self._memo.name_matches(node.value, "typing.Union") and any(
                    item is None or (isinstance(item, expr) and self._memo.name_matches(item, *anytype_names)) for item in items
                ):
                    return None
        ...
        with self._use_memo(node):
            ...
            if arg_annotations:
                annotations_dict = Dict(
                    ...
                    values=[Tuple([Name(key, ctx=Load()), annotation], ctx=Load()) for key, annotation in arg_annotations.items()],
                )
