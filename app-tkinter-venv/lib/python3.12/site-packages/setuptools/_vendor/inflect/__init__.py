...
pl_sb_singular_s = enclose("|".join(pl_sb_singular_s_complete + [f".*{w}" for w in pl_sb_singular_s_endings] + pl_sb_singular_s_es))
...
plverb_special_s = enclose("|".join([pl_sb_singular_s] + pl_sb_uninflected_s + list(pl_sb_irregular_s) + ["(.*[csx])is", "(.*)ceps", "[A-Z].*s"]))
...
class engine:
    ...
    def _string_to_substitute(self, mo: Match, methods_dict: Dict[str, Callable]) -> str:
        ...
        kwargs_list = {kw.arg: self._get_value_from_ast(kw.value) for kw in a_tree.body[0].value.keywords}  # type: ignore[attr-defined]
