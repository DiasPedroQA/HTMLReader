        ...
        for splitter in ("import ", "cimport ", ".", "as "):
            ...
            if re.search(exp, line_without_comment) and not line_without_comment.strip().startswith(splitter):
                ...
                if comment and not (config.use_parentheses and "noqa" in comment):
                    _comma_maybe = (
                        "," if (config.include_trailing_comma and config.use_parentheses and not line_without_comment.rstrip().endswith(",")) else ""
                    )
                ...
                if config.use_parentheses:
                    if splitter == "as ":
                        ...
                        output = f"{content}{splitter}({noqa_comment}" f"{line_separator}{cont_line}{_comma}{_separator})"
