...
class IntroducedSyntaxErrors(ISortError):
    ...
    def __init__(self, file_path: str):
        super().__init__(f"isort introduced syntax errors when attempting to sort the imports contained within " f"{file_path}.")
        self.file_path = file_path
...
class ProfileDoesNotExist(ISortError):
    ...
    def __init__(self, profile: str):
        super().__init__(f"Specified profile of {profile} does not exist. " f"Available profiles: {','.join(profiles)}.")
...
class SortingFunctionDoesNotExist(ISortError):
    ...
    def __init__(self, sort_order: str, available_sort_orders: List[str]):
        super().__init__(f"Specified sort_order of {sort_order} does not exist. " f"Available sort_orders: {','.join(available_sort_orders)}.")
...
class LiteralSortTypeMismatch(ISortError):
    ...
    def __init__(self, kind: type, expected_kind: type):
        super().__init__(f"isort was told to sort a literal of type {expected_kind} but was given " f"a literal of type {kind}.")
        self.kind = kind
