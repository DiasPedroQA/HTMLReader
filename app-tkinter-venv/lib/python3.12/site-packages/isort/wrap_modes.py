...
def hanging_indent(**interface: Any) -> str:
    ...
    while interface["imports"]:
        ...
        if len(next_statement.split(interface["line_separator"])[-1]) > line_length_limit:
            next_statement = (
                _hanging_indent_end_line(interface["statement"] + ",") + f"{interface['line_separator']}{interface['indent']}{next_import}"
            )
        interface["statement"] = next_statement
...
def _vertical_grid_common(need_trailing_char: bool, **interface: Any) -> str:
    ...
    while interface["imports"]:
        ...
        if current_line_length > interface["line_length"]:
            next_statement = f"{interface['statement']},{interface['line_separator']}" f"{interface['indent']}{next_import}"
        interface["statement"] = next_statement
...
WrapModes = enum.Enum("WrapModes", {wrap_mode: index for index, wrap_mode in enumerate(_wrap_modes.keys())})  # type: ignore
