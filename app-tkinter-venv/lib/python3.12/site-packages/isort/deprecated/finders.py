...
class KnownPatternFinder(BaseFinder):
    def __init__(self, config: Config) -> None:
        ...
        for placement in reversed(config.sections):
            ...
            known_patterns = [pattern for known_pattern in known_patterns for pattern in self._parse_known_pattern(known_pattern)]
    ...
    def find(self, module_name: str) -> Optional[str]:
        for prefix in self.paths:
            ...
            is_module = (
                ...
                or any(exists_case_sensitive(package_path + ext_suffix) for ext_suffix in importlib.machinery.EXTENSION_SUFFIXES)
            ...
            if is_module or is_package:
                if "site-packages" in prefix or "dist-packages" in prefix or (self.virtual_env and self.virtual_env_src in prefix):
        ...
        for fname in os.listdir(path):
            ...
            if os.path.isdir(full_path):
                for subfile_name in os.listdir(full_path):
                    results.extend(
                        os.path.join(full_path, subfile_name) for ext in cls.ext if subfile_name.endswith(ext)  # type: ignore[attr-defined]
                    )
                continue
    ...
    def find(self, module_name: str) -> Optional[str]:
        for finder in self.finders:
            try:
                ...
                if self.verbose:
                    print(f"{finder.__class__.__name__} encountered an error ({exception}) while " f"trying to identify the {module_name} module")
        return None
