    ...
    for index, line in enumerate(chain(input_stream, (None,))):
        if line is None:
            ...
            if (
                ...
                in_top_comment = True
            elif in_top_comment and (not line.startswith("#") or stripped_line in config.section_comments or stripped_line in CODE_SORT_COMMENTS):
                ...
                if isort_off:
                    ...
                    if import_statement.lstrip().startswith("from") and "import" not in import_statement:
                        ...
                        if (
                            ...
                            or " cimport*" in import_statement
                            or " cimport(" in import_statement
                            or (".cimport" in import_statement and "cython.cimports" not in import_statement)  # Allow pure python imports. See #2062
                        ):
                            cimport_statement = True

                        if cimport_statement != cimports or (
                            new_indent != indent and import_section and (not did_contain_imports or len(new_indent) < len(indent))
                        ):
                            indent = new_indent
                            if import_section:
        ...
        if not_imports:
            ...
            if import_section:
                ...
                if not contains_imports:
                    ...
                    if first_import_section and not import_section.lstrip(line_separator).startswith(COMMENT_INDICATORS):
                    ...
                    if indent:
                        import_section = "".join(line[len(indent) :] for line in import_section.splitlines(keepends=True))
                    ...
                    if not (import_section.strip() and not sorted_import_section):
                        if indent:
                            sorted_import_section = leading_whitespace + textwrap.indent(sorted_import_section, indent).strip() + trailing_whitespace
...
def _has_changed(before: str, after: str, line_separator: str, ignore_whitespace: bool) -> bool:
    if ignore_whitespace:
        return remove_whitespace(before, line_separator=line_separator).strip() != remove_whitespace(after, line_separator=line_separator).strip()
