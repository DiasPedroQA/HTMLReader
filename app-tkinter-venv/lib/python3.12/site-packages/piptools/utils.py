...
def _validate_config(
    ...
    for key, value in config.items():
        ...
        associated_params = (cli_params[key] for cli_params in (compile_cli_params, sync_cli_params) if key in cli_params)
...
def select_config_file(src_files: tuple[str, ...]) -> Path | None:
    ...
    return config_file_path.relative_to(working_directory) if is_path_relative_to(config_file_path, working_directory) else config_file_path
...
def _invert_negative_bool_options_in_config(ctx: click.Context, config: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in config.items():
        ...
        new_value = not value if long_option.startswith("--no-") and long_option not in ONLY_NEGATIVE_OPTIONS and isinstance(value, bool) else value
