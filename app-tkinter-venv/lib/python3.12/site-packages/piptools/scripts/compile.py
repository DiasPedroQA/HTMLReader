...
def cli(
    ...
    if len(src_files) == 0:
        for file_path in DEFAULT_REQUIREMENTS_FILES:
            ...
            raise click.BadParameter(("If you do not specify an input file, the default is one of: {}").format(", ".join(DEFAULT_REQUIREMENTS_FILES)))
    ...
    if resolver_name == "legacy":
        log.warning("WARNING: the legacy dependency resolver is deprecated and will be removed" " in future versions of pip-tools.")
    ...
    for src_file in src_files:
        ...
        if not is_setup_file and build_deps_targets:
            msg = "--build-deps-for and --all-build-deps can be used only with the " "setup.py, setup.cfg and pyproject.toml specs."
            raise click.BadParameter(msg)
