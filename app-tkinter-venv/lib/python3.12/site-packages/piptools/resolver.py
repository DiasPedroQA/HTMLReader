...
class LegacyResolver(BaseResolver):
    ...
    def get_best_match(self, ireq: InstallRequirement) -> InstallRequirement:
        ...
        log.debug("found candidate {} (constraint was {})".format(format_requirement(best_match), format_specifier(ireq)))
    ...
    def _ireqs_of_dependencies(self, ireq: InstallRequirement, dependency_strings: list[str]) -> Iterator[InstallRequirement]:
        log.debug(
            "{:25} requires {}".format(
...
class BacktrackingResolver(BaseResolver):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._constraints_map = {ireq_key: combine_install_requirements(ireqs) for ireq_key, ireqs in constraints_sets.items()}
    ...
    def resolve(self, max_rounds: int = 10) -> set[InstallRequirement]:
        ...
        ``InstallRequirement``'s.
        """
        with update_env_context_manager(PIP_EXISTS_ACTION="i"), get_build_tracker() as build_tracker, global_tempdir_manager(), indent_log():
    ...
    def _get_install_requirement_from_candidate(self, candidate: Candidate, reverse_dependencies: dict[str, set[str]]) -> InstallRequirement | None:
