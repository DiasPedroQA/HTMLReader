...
class PyPIRepository(BaseRepository):
    ...
    def find_best_match(self, ireq: InstallRequirement, prereleases: bool | None = None) -> InstallRequirement:
        ...
        matching_versions = ireq.specifier.filter((candidate.version for candidate in all_candidates), prereleases=prereleases)

        matching_candidates = list(itertools.chain.from_iterable(candidates_by_version[ver] for ver in matching_versions))
    ...
    def _get_project(self, ireq: InstallRequirement) -> Any:
        ...
        """
        package_indexes = (PackageIndex(url=index_url, file_storage_domain="") for index_url in self.finder.search_scope.index_urls)
    ...
    def _get_req_hashes(self, ireq: InstallRequirement) -> set[str]:
        ...
        pypi_hashes = {pypi_hashes_by_link[candidate.link.url] for candidate in matching_candidates if candidate.link.url in pypi_hashes_by_link}
        local_hashes = {self._get_file_hash(candidate.link) for candidate in matching_candidates if candidate.link.url not in pypi_hashes_by_link}
