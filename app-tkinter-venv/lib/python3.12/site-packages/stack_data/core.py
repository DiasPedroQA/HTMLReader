...
from typing import Any, Callable, Iterable, Iterator, List, Mapping, NamedTuple, Optional, Sequence, Tuple, Union
...
class Line(object):
    ...
    def __repr__(self):
        return "<{self.__class__.__name__} {self.lineno} (current={self.is_current}) " "{self.text!r} of {self.frame_info.filename}>".format(
            self=self
        )
    ...
    def render(
        self, markers: Iterable[MarkerInLine] = (), *, strip_leading_indent: bool = True, pygmented: bool = False, escape_html: bool = False
    ) -> str:
        """
...
class FrameInfo(object):
    ...
    @classmethod
    def stack_data(
        cls, frame_or_tb: Union[FrameType, TracebackType], options: Optional[Options] = None, *, collapse_repeated_frames: bool = True
    ...
    def variables(self) -> List[Variable]:
        ...
        scope = self.scope
        node_values = [pair for pair in evaluator.find_expressions(scope) if is_expression_interesting(*pair)]  # type: List[Tuple[ast.AST, Any]]
