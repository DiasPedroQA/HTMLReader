...
class ASTStubGenerator(BaseStubGenerator, mypy.traverser.TraverserVisitor):
    ...
    def visit_func_def(self, o: FuncDef) -> None:
        ...
        if is_dataclass_generated:
            ...
            return
        if self.is_private_name(o.name, o.fullname) or self.is_not_in_all(o.name) or (self.is_recorded_name(o.name) and not o.is_overload):
            self.clear_decorators()
            return
        ...
        for output in self.format_func_def(sigs, is_coroutine=o.is_coroutine, decorators=self._decorators, docstring=ctx.docstring):
            self.add(output + "\n")
        ...
        for lvalue in o.lvalues:
            if isinstance(lvalue, NameExpr) and isinstance(o.rvalue, CallExpr):
                ...
                if self.is_typeddict(o.rvalue):
                    ...
                    continue
            if isinstance(lvalue, NameExpr) and self.is_alias_expression(o.rvalue) and not self.is_private_name(lvalue.name):
    ...
    def is_alias_expression(self, expr: Expression, top_level: bool = True) -> bool:
        ...
        if isinstance(expr, CallExpr) and self.get_fullname(expr.callee) in TYPE_VAR_LIKE_NAMES:
            ...
            or (  # Also some known aliases that could be member expression
                ...
                and self.get_fullname(expr.base).startswith(("builtins.", "typing.", "typing_extensions.", "collections.abc."))
            )
        ):
            ...
            return self.is_alias_expression(expr.left, top_level=False) and self.is_alias_expression(expr.right, top_level=False)
        else:
            return False
    ...
    def visit_import_from(self, o: ImportFrom) -> None:
        ...
        if self.module_name:
            full_module, ok = mypy.util.correct_relative_import(self.module_name, relative, module, self.path.endswith(".__init__.py"))
            if not ok:
                full_module = module
        else:
            full_module = module
        ...
        if self._all_:
            ...
            names = [name for name, alias in o.names if name in self._all_ and alias is None and name not in self.IGNORED_DUNDERS]
...
def collect_build_targets(options: Options, mypy_opts: MypyOptions) -> tuple[list[StubSource], list[StubSource], list[StubSource]]:
    ...
    if options.packages or options.modules:
        if options.no_import:
            py_modules = find_module_paths_using_search(options.modules, options.packages, options.search_path, options.pyversion)
            c_modules: list[StubSource] = []
        else:
            # Using imports is the default, since we can also find C modules.
            py_modules, c_modules = find_module_paths_using_imports(options.modules, options.packages, options.verbose, options.quiet)
    else:
...
def find_module_paths_using_imports(modules: list[str], packages: list[str], verbose: bool, quiet: bool) -> tuple[list[StubSource], list[StubSource]]:
    ...
    with ModuleInspect() as inspect:
        ...
        modules = [mod for mod in modules if not is_non_library_module(mod)]  # We don't want to run any tests or scripts
        for mod in modules:
            try:
...
def parse_options(args: list[str]) -> Options:
    ...
    parser.add_argument(
        "--no-import",
        action="store_true",
        help="don't import the modules, just parse and analyze them " "(doesn't work with C extension modules and might not " "respect __all__)",
    )
    parser.add_argument(
        "--no-analysis",
        "--parse-only",
    ...
    parser.add_argument(
        "--include-private",
        action="store_true",
        help="generate stubs for objects and members considered private " "(single leading underscore and no trailing underscores)",
    )
    parser.add_argument(
        "--export-less",
        action="store_true",
