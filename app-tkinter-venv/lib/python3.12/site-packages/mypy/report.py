...
class LineCountReporter(AbstractReporter):
    ...
    def on_finish(self) -> None:
        counts: list[tuple[tuple[int, int, int, int], str]] = sorted(((c, p) for p, c in self.counts.items()), reverse=True)
...
class MemoryXmlReporter(AbstractReporter):
    ...
    def on_file(
        ...
        transform_pi = etree.ProcessingInstruction("xml-stylesheet", f'type="text/xsl" href="{pathname2url(xslt_path)}"')
