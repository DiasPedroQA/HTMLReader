...
class Attribute:
    ...
    def serialize(self) -> JsonDict:
        ...
        return {
            ...
            "converter_init_type": (self.converter.init_type.serialize() if self.converter and self.converter.init_type else None),
    ...
    def deserialize(cls, info: TypeInfo, data: JsonDict, api: SemanticAnalyzerPluginInterface) -> Attribute:
        ...
        converter_init_type = deserialize_and_fixup_type(raw_converter_init_type, api) if raw_converter_init_type else None
...
def attr_class_maker_callback_impl(
    ...
    if frozen:
        ...
        hashable: bool | None = _get_decorator_bool_argument(ctx, "hash", True) and _get_decorator_bool_argument(ctx, "unsafe_hash", True)
    else:
...
def _attributes_from_assignment(ctx: mypy.plugin.ClassDefContext, stmt: AssignmentStmt, auto_attribs: bool, kw_only: bool) -> Iterable[Attribute]:
    ...
    for lvalue in stmt.lvalues:
        ...
        for lhs, rvalue in zip(lvalues, rvalues):
            # Check if the right hand side is a call to an attribute maker.
            if isinstance(rvalue, CallExpr) and isinstance(rvalue.callee, RefExpr) and rvalue.callee.fullname in attr_attrib_makers:
...
def _cleanup_decorator(stmt: Decorator, attr_map: dict[str, Attribute]) -> None:
    ...
    for func_decorator in stmt.decorators:
        if isinstance(func_decorator, MemberExpr) and isinstance(func_decorator.expr, NameExpr) and func_decorator.expr.name in attr_map:
...
def _add_attrs_magic_attribute(ctx: mypy.plugin.ClassDefContext, attrs: list[tuple[str, Type | None]]) -> None:
    ...
    attributes_types: list[Type] = [ctx.api.named_type_or_none("attr.Attribute", [attr_type or any_type]) or any_type for _, attr_type in attrs]
    fallback_type = ctx.api.named_type("builtins.tuple", [ctx.api.named_type_or_none("attr.Attribute", [any_type]) or any_type])
