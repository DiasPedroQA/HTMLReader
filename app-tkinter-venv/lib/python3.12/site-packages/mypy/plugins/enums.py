...
def enum_member_callback(ctx: mypy.plugin.FunctionContext) -> Type:
    ...
    if ctx.arg_types or ctx.arg_types[0]:
        ...
        if isinstance(arg, Instance) and arg.last_known_value and isinstance(proper_return, Instance) and len(proper_return.args) == 1:
...
def enum_value_callback(ctx: mypy.plugin.AttributeContext) -> Type:
    ...
    if enum_field_name is None:
        ...
        if isinstance(ctx.type, Instance):
            ...
            all_same_value_type = all(proper_type is not None and proper_type == underlying_type for proper_type in proper_types)
            ...
            all_equivalent_types = all(proper_type is not None and is_equivalent(proper_type, underlying_type) for proper_type in proper_types)
