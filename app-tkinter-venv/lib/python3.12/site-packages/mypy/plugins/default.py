    ...
    if isinstance(ctx.type, TypedDictType) and len(ctx.arg_types) >= 1 and len(ctx.arg_types[0]) == 1:
        ...
        for key in keys:
            ...
            if len(ctx.arg_types) == 1:
                ...
                default_arg = ctx.args[1][0]
                if isinstance(default_arg, DictExpr) and len(default_arg.items) == 0 and isinstance(value_type, TypedDictType):
...
def typed_dict_setdefault_callback(ctx: MethodContext) -> Type:
    ...
    if isinstance(ctx.type, TypedDictType) and len(ctx.arg_types) == 2 and len(ctx.arg_types[0]) == 1 and len(ctx.arg_types[1]) == 1:
        key_expr = ctx.args[0][0]
        keys = try_getting_str_literals(key_expr, ctx.arg_types[0][0])
        if keys is None:
            ctx.api.fail(
    ...
    if isinstance(ctx.type, TypedDictType) and len(signature.arg_types) == 1:
        ...
        if ctx.args and ctx.args[0]:
            ...
            for td in possible_tds:
                item = arg_type.copy_modified(required_keys=(arg_type.required_keys | td.required_keys) & arg_type.items.keys())
