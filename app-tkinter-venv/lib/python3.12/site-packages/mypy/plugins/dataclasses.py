...
class DataclassTransformer:
    ...
    def transform(self) -> bool:
        ...
        if decorator_arguments["init"] and ("__init__" not in info.names or info.names["__init__"].plugin_generated) and attributes:
            args = [attr.to_argument(info, of="__init__") for attr in attributes if attr.is_in_init and not self._is_kw_only_type(attr.type)]
        ...
        if (
            ...
            literals: list[Type] = [LiteralType(attr.name, str_type) for attr in attributes if attr.is_in_init and not attr.kw_only]
    ...
    def _collect_field_args(self, expr: Expression) -> tuple[bool, dict[str, Expression]]:
        ...
        """
        if isinstance(expr, CallExpr) and isinstance(expr.callee, RefExpr) and expr.callee.fullname in self._spec.field_specifiers:
...
def _get_transform_spec(reason: Expression) -> DataclassTransformSpec:
    ...
    assert spec is not None, (
        "trying to find dataclass transform spec, but reason is neither dataclasses.dataclass nor " "decorated with typing.dataclass_transform"
    )
    return spec
