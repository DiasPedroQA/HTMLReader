...
class ExpressionChecker(ExpressionVisitor[Type], ExpressionCheckerSharedApi):
        ...
        if isinstance(e.callee, NameExpr) and e.callee.name in ("isinstance", "issubclass") and len(e.args) == 2:
            for typ in mypy.checker.flatten(e.args[1]):
                ...
                if (isinstance(typ, IndexExpr) and isinstance(typ.analyzed, (TypeApplication, TypeAliasExpr))) or (
                    ...
                    and not (isinstance(union_target := get_proper_type(node.node.target), UnionType) and union_target.uses_pep604_syntax)
                ):
        ...
        if not (callee.required_keys <= always_present_keys and actual_keys <= callee.items.keys()):
            if not (actual_keys <= callee.items.keys()):
                self.msg.unexpected_typeddict_keys(
                    callee,
                    expected_keys=[key for key in callee.items.keys() if key in callee.required_keys or key in actual_keys],
        ...
        if typename in self.item_args and methodname in self.item_args[typename] and e.arg_kinds == [ARG_POS]:
            ...
            if isinstance(arg_type, Instance):
                ...
                if arg_typename in self.container_args[typename][methodname]:
                    if all(mypy.checker.is_valid_inferred_type(item_type, self.chk.options) for item_type in arg_type.args):
    ...
    def check_call_expr_with_callee_type(
        ...
        if isinstance(e.callee, (NameExpr, MemberExpr)):
            ...
            self.chk.warn_deprecated_overload_item(node, e, target=callee_type, selftype=object_type)
    ...
    def infer_function_type_arguments(
        ...
        if self.chk.in_checked_function():
            ...
            if not self.chk.options.old_type_inference and any(
                a is None or isinstance(get_proper_type(a), UninhabitedType) or set(get_type_vars(a)) & set(callee_type.variables)
                ...
                inferred_args = [
                    (expand_type(a, {v.id: unknown for v in list(callee_type.variables) + free_vars}) if a is not None else None)
    ...
    def check_argument_count(
        ...
        for i, kind in enumerate(callee.arg_kinds):
            ...
            if kind.is_required() and not mapped_args and not is_unexpected_arg_error:
                ...
                if self.chk.in_checked_function() or isinstance(get_proper_type(actual_types[mapped_args[0]]), TupleType):
                    self.msg.duplicate_argument_value(callee, i, context)
                    ok = False
            elif kind.is_named() and mapped_args and actual_kinds[mapped_args[0]] not in [nodes.ARG_NAMED, nodes.ARG_STAR2]:
                ...
                if not mapped_args and callee.special_sig != "partial":
                    ...
                    paramspec_entries = sum(isinstance(get_proper_type(actual_types[k]), ParamSpecType) for k in mapped_args)
        ...
        *,
        ...
        if isinstance(left, Instance) and isinstance(right, Instance):
            ...
            if left_name in OVERLAPPING_TYPES_ALLOWLIST and right_name in OVERLAPPING_TYPES_ALLOWLIST:
                ...
                return self.dangerous_comparison(left.args[0], right.args[0], seen_types=seen_types) or self.dangerous_comparison(
            ...
            if local_errors.has_new_errors():
                ...
                if isinstance(obj, Instance) and isinstance(defn := obj.type.get_method(name), OverloadedFuncDef):
                    for item in defn.items:
                        if isinstance(item, Decorator) and isinstance(typ := item.func.type, CallableType) and bind_self(typ) == result[1]:
    ...
    def visit_typeddict_index_expr(self, td_type: TypedDictType, index: Expression, setitem: bool = False) -> tuple[Type, set[str]]:
    ...
    def visit_cast_expr(self, expr: CastExpr) -> Type:
        ...
        if options.warn_redundant_casts and not is_same_type(target_type, AnyType(TypeOfAny.special_form)) and is_same_type(source_type, target_type):
        ...
        for tv, arg in zip(t.variables, args):
            if isinstance(tv, ParamSpecType):
                if not isinstance(get_proper_type(arg), (Parameters, ParamSpecType, AnyType, UnboundType)):
                    self.chk.fail(
                        "Can only replace ParamSpec with a parameter types list or" f" another ParamSpec, got {format_type(arg, self.chk.options)}",
                        ctx,
                    )
    ...
    def visit_tuple_expr(self, e: TupleExpr) -> Type:
        ...
        if isinstance(type_context, UnionType):
            tuples_in_context = [
                ...
                if (isinstance(t, TupleType) and self.tuple_context_matches(e, t)) or is_named_instance(t, TUPLE_LIKE_INSTANCE_NAMES)
            ]
    ...
    def check_for_comp(self, e: GeneratorExpr | DictionaryComprehension) -> None:
        ...
        for index, sequence, conditions, is_async in zip(e.indices, e.sequences, e.condlists, e.is_async):
            if is_async:
                ...
                if isinstance(get_proper_type(sequence_type), UninhabitedType) and isinstance(index, NameExpr) and index.name == "_":
...
def is_duplicate_mapping(mapping: list[int], actual_types: list[Type], actual_kinds: list[ArgKind]) -> bool:
    return (
        ...
        and not (len(mapping) == 2 and actual_kinds[mapping[0]] == nodes.ARG_STAR and actual_kinds[mapping[1]] == nodes.ARG_STAR2)
        ...
        # **kwargs).
        and not all(actual_kinds[m] == nodes.ARG_STAR2 and not isinstance(get_proper_type(actual_types[m]), TypedDictType) for m in mapping)
    )
