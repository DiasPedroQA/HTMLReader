    ...
    def assign_type(self, expr: Expression, type: Type, declared_type: Type | None) -> None:
        ...
        if isinstance(p_type, AnyType):
            ...
            if isinstance(expr, RefExpr) and isinstance(expr.node, Var) and expr.node.is_inferred:
                ...
                self.put(expr, type)
            elif isinstance(p_declared, UnionType) and any(isinstance(get_proper_type(item), NoneType) for item in p_declared.items):
            ...
            elif isinstance(p_declared, UnionType) and any(isinstance(get_proper_type(item), AnyType) for item in p_declared.items):
