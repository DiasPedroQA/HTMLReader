...
class TypeAnalyser(SyntheticTypeVisitor[Type], TypeAnalyzerPluginInterface):
    ...
    def check_and_warn_deprecated(self, info: TypeInfo, ctx: Context) -> None:
        ...
        if (
            ...
            and not any(info.fullname == p or info.fullname.startswith(f"{p}.") for p in self.options.deprecated_calls_exclude)
        ):
    ...
    def analyze_unbound_type_without_type_info(self, t: UnboundType, sym: SymbolTableNode, defining_literal: bool) -> Type:
        ...
        if isinstance(sym.node, Var):
            ...
            if sym.node.is_new_style:
                ...
                short = name.split(".")[-1]
                notes.append(f'(Hint: Use "Generic[{short}]" or "Protocol[{short}]" base class' f' to bind "{short}" inside a class)')
    ...
    def visit_typeddict_type(self, t: TypedDictType) -> Type:
        ...
        if t.fallback.type is MISSING_FALLBACK:  # anonymous/inline TypedDict
            if INLINE_TYPEDDICT not in self.options.enable_incomplete_feature:
                self.fail(
                    "Inline TypedDict is experimental," " must be enabled with --enable-incomplete-feature=InlineTypedDict",
                    t,
                )
    ...
    def analyze_callable_args_for_paramspec(self, callable_args: Type, ret_type: Type, fallback: Instance) -> CallableType | None:
        ...
        if not isinstance(tvar_def, ParamSpecType):
            ...
            return None
        elif self.defining_alias and self.not_declared_in_type_params(tvar_def.name) and tvar_def not in self.allowed_alias_tvars:
            if self.python_3_12_type_alias:
            ...
            if isinstance(arg, CallableArgument):
                ...
                if found is None:
                    ...
                    if arg.name is not None and kind.is_star():
                        ...
                        return None
            elif isinstance(arg, UnboundType) and self.refers_to_full_names(arg, UNPACK_TYPE_NAMES) or isinstance(arg, UnpackType):
                if seen_unpack:
    ...
    def analyze_literal_param(self, idx: int, arg: Type, ctx: Context) -> list[Type] | None:
        ...
        if isinstance(arg, ProperType) and isinstance(arg, (UnboundType, UnionType)) and arg.original_str_expr is not None:
...
def fix_type_var_tuple_argument(t: Instance) -> None:
    if t.type.has_type_var_tuple_type:
        ...
        args[t.type.type_var_tuple_prefix] = UnpackType(Instance(tvt.tuple_fallback.type, [args[t.type.type_var_tuple_prefix]]))
        t.args = tuple(args)
...
def instantiate_type_alias(
    ...
    *,
        ...
        if not correct:
            if use_standard_error:
                ...
                if node.tvar_tuple_index is not None:
                    msg = "Bad number of arguments for type alias," f" expected at least {min_tv_count}, given {act_len}"
                elif min_tv_count != max_tv_count:
                    msg = "Bad number of arguments for type alias," f" expected between {min_tv_count} and {max_tv_count}, given {act_len}"
                else:
    ...
    if isinstance(typ.alias.target, Instance) and typ.alias.target.type.fullname == "mypy_extensions.FlexibleAlias":  # type: ignore[misc]
...
class DivergingAliasDetector(TrivialSyntheticTypeTranslator):
    ...
    def visit_type_alias_type(self, t: TypeAliasType) -> Type:
        ...
        if t.alias in self.seen_nodes:
            for arg in t.args:
                if not (isinstance(arg, TypeVarLikeType) or isinstance(arg, UnpackType) and isinstance(arg.type, TypeVarLikeType)) and has_type_vars(
                    arg
                ):
                    self.diverging = True
                    return t
...
def validate_instance(t: Instance, fail: MsgCallback, empty_tuple_index: bool) -> bool:
    ...
    if any(unknown_unpack(a) for a in t.args):
        ...
        return False
    if t.type.has_type_var_tuple_type:
        min_tv_count = sum(not tv.has_default() and not isinstance(tv, TypeVarTupleType) for tv in t.type.defn.type_vars)
    ...
    def visit_unbound_type(self, t: UnboundType) -> Type:
        ...
        if sym is not None:
            ...
            if isinstance(sym.node, TypeVarLikeExpr):
                self.api.fail(
                    f'Type parameter "{self.tvar_expr_name}" has a default type ' "that refers to one or more type variables that are out of scope",
                    self.context,
                )
                return AnyType(TypeOfAny.from_error)
        return super().visit_unbound_type(t)
