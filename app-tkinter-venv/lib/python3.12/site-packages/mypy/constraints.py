        ...
        if isinstance(callee.arg_types[i], UnpackType):
            ...
            if isinstance(unpacked_type, TypeVarTupleType):
                ...
                if isinstance(inner_unpacked_type, TypeVarTupleType):
                    ...
                    assert isinstance(inner_unpacked_type, Instance) and inner_unpacked_type.type.fullname == "builtins.tuple"
            ...
            for actual in actuals:
                ...
                if param_spec and callee.arg_kinds[i] in (ARG_STAR, ARG_STAR2):
                    ...
                    if not incomplete_star_mapping:
                        param_spec_arg_types.append(mapper.expand_actual_type(actual_arg_type, arg_kinds[actual], None, arg_kinds[actual]))
                        actual_kind = arg_kinds[actual]
                        param_spec_arg_kinds.append(ARG_POS if actual_kind not in (ARG_STAR, ARG_STAR2) else actual_kind)
...
def infer_constraints(template: Type, actual: Type, direction: int, skip_neg_op: bool = False) -> list[Constraint]:
    ...
    """
    if any(
        get_proper_type(template) == get_proper_type(t) and get_proper_type(actual) == get_proper_type(a) for (t, a) in reversed(type_state.inferring)
    ):
        return []
...
def _is_type_type(tp: ProperType) -> TypeGuard[TypeType | UnionType]:
    ...
    """
    return isinstance(tp, TypeType) or isinstance(tp, UnionType) and all(isinstance(get_proper_type(o), TypeType) for o in tp.items)
...
def is_same_constraint(c1: Constraint, c2: Constraint) -> bool:
    ...
    return c1.type_var == c2.type_var and (c1.op == c2.op or skip_op_check) and mypy.subtypes.is_same_type(c1.target, c2.target)
...
def _is_similar_constraints(x: list[Constraint], y: list[Constraint]) -> bool:
    ...
    for c1 in x:
        ...
        for c2 in y:
            ...
            skip_op_check = isinstance(get_proper_type(c1.target), AnyType) or isinstance(get_proper_type(c2.target), AnyType)
...
class ConstraintBuilderVisitor(TypeVisitor[list[Constraint]]):
    ...
    def visit_type_var(self, template: TypeVarType) -> list[Constraint]:
        assert False, "Unexpected TypeVarType in ConstraintBuilderVisitor" " (should have been handled in infer_constraints)"
    ...
    def visit_instance(self, template: Instance) -> list[Constraint]:
        ...
        if isinstance(actual, CallableType) and actual.fallback is not None:
            if actual.is_type_obj() and template.type.is_protocol and self.direction == SUPERTYPE_OF:
                ...
                if isinstance(ret_type, Instance):
                    res.extend(self.infer_constraints_from_protocol_members(ret_type, template, ret_type, template, class_obj=True))
        ...
        if isinstance(actual, TypeType) and template.type.is_protocol:
            if self.direction == SUPERTYPE_OF:
                ...
                if isinstance(a_item, Instance):
                    res.extend(self.infer_constraints_from_protocol_members(a_item, template, a_item, template, class_obj=True))
                ...
                if isinstance(a_item, Instance) and a_item.type.metaclass_type:
                    res.extend(self.infer_constraints_from_protocol_members(a_item.type.metaclass_type, template, actual, template))
        ...
        if isinstance(actual, TupleType) and is_named_instance(template, TUPLE_LIKE_INSTANCE_NAMES) and self.direction == SUPERTYPE_OF:
            ...
            if param_spec is None:
                ...
                if not template.is_ellipsis_args:
                    ...
                    if unpack_present is not None and not cactual.param_spec():
                        ...
                        unpack_constraints = build_constraints_for_simple_unpack(template_types, actual_types, neg_op(self.direction))
        ...
        if isinstance(actual, TupleType) or is_varlength_tuple:
            ...
            if unpack_index is not None:
                if is_varlength_tuple:
                    ...
                    unpack_constraints = build_constraints_for_simple_unpack(template.items, actual.items, self.direction)
                ...
                if a_unpack_index is not None:
                    ...
                    if len(actual.items) + 1 <= len(template.items):
                        ...
                        t_prefix, t_middle, t_suffix = split_with_prefix_and_suffix(tuple(template.items), a_prefix_len, a_suffix_len)
    ...
    def visit_union_type(self, template: UnionType) -> list[Constraint]:
        assert False, "Unexpected UnionType in ConstraintBuilderVisitor" " (should have been handled in infer_constraints)"
