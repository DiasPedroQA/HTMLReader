...
class TypedDictAnalyzer:
    ...
    def analyze_typeddict_classdef(self, defn: ClassDef) -> tuple[bool, TypeInfo | None]:
        ...
        if len(defn.base_type_exprs) == 1 and isinstance(defn.base_type_exprs[0], RefExpr) and defn.base_type_exprs[0].fullname in TPDICT_NAMES:
            ...
            info = self.build_typeddict_typeinfo(defn.name, field_types, required_keys, readonly_keys, defn.line, existing_info)
        ...
        for expr in defn.base_type_exprs:
            ...
            elif isinstance(expr, RefExpr) and self.is_typeddict(expr) or isinstance(expr, IndexExpr) and self.is_typeddict(expr.base):
        ...
        (new_field_types, new_statements, new_required_keys, new_readonly_keys) = self.analyze_typeddict_classdef_fields(defn, oldfields=field_types)
    ...
    def check_typeddict(self, node: Expression, var_name: str | None, is_func_scope: bool) -> tuple[bool, TypeInfo | None, list[TypeVarLikeType]]:
        ...
        if not ok:
            ...
            required_keys = {
                ...
                if (total or (isinstance(t, RequiredType) and t.required)) and not (isinstance(t, RequiredType) and not t.required)
            }
            readonly_keys = {field for (field, t) in zip(items, types) if isinstance(t, ReadOnlyType)}
            types = [t.item if isinstance(t, (RequiredType, ReadOnlyType)) else t for t in types]  # unwrap Required[T] or ReadOnly[T] to just T
    ...
    def parse_typeddict_fields_with_types(self, dict_items: list[tuple[Expression | None, Expression]]) -> tuple[list[str], list[Type], bool] | None:
