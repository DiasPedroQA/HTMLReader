...
def verify_mypyfile(stub: nodes.MypyFile, runtime: MaybeMissing[types.ModuleType], object_path: list[str]) -> Iterator[Error]:
    ...
    to_check = {m for m, o in stub.names.items() if not o.module_hidden and (not is_probably_private(m) or hasattr(runtime, m))}
...
def verify_typeinfo(stub: nodes.TypeInfo, runtime: MaybeMissing[type[Any]], object_path: list[str]) -> Iterator[Error]:
    ...
    for entry in sorted(to_check):
        ...
        if runtime_attr is not MISSING and type(runtime) is not runtime and getattr(runtime_attr, "__objclass__", None) is type(runtime):
    ...
    if runtime_arg.default is not inspect.Parameter.empty:
        if stub_arg.kind.is_required():
            ...
            if stub_arg.initializer is not None:
                ...
                if stub_default is not UNKNOWN and stub_default is not ... and runtime_arg.default is not UNREPRESENTABLE:
                    defaults_match = True
    ...
    def from_overloadedfuncdef(stub: nodes.OverloadedFuncDef) -> Signature[nodes.Argument]:
        ...
        for func in map(_resolve_funcitem_from_decorator, stub.items):
            ...
            for index, arg in enumerate(args):
                ...
                name = (
                    f"__{index}"
                    if arg.variable.name.startswith("__") or arg.pos_only or assume_positional_only or arg.variable.name.strip("_") == "self"
...
def _verify_signature(stub: Signature[nodes.Argument], runtime: Signature[inspect.Parameter], function_name: str) -> Iterator[str]:
    ...
    for stub_arg, runtime_arg in zip(stub.pos, runtime.pos):
        ...
        if (
            ...
            yield (f'stub argument "{stub_arg.variable.name}" should be positional-only ' f'(add "/", e.g. "{runtime_arg.name}, /")')
        if (
            runtime_arg.kind != inspect.Parameter.POSITIONAL_ONLY
    ...
    for arg in sorted(set(runtime.kwonly) - set(stub.kwonly)):
        if arg in {stub_arg.variable.name for stub_arg in stub.pos}:
            # Don't report this if we've reported it before
            if not (runtime.varpos is None and arg in {stub_arg.variable.name for stub_arg in stub.pos[len(runtime.pos) :]}):
...
def verify_var(stub: nodes.Var, runtime: MaybeMissing[Any], object_path: list[str]) -> Iterator[Error]:
    ...
    if stub.is_initialized_in_class and is_read_only_property(runtime) and (stub.is_settable_property or not stub.is_property):
...
def verify_overloadedfuncdef(stub: nodes.OverloadedFuncDef, runtime: MaybeMissing[Any], object_path: list[str]) -> Iterator[Error]:
    ...
    for message in _verify_signature(stub_sig, runtime_sig, function_name=stub.name):
        ...
        if "has a default value of type" in message:
            message += ". This is often caused by overloads failing to account for explicitly passing " "in the default value."
        yield Error(
            object_path,
            "is inconsistent, " + message,
            stub,
...
def _verify_final_method(stub: nodes.FuncDef, runtime: Any, static_runtime: MaybeMissing[Any]) -> Iterator[str]:
    if stub.is_final:
        return
    if getattr(runtime, "__final__", False) or (static_runtime is not MISSING and getattr(static_runtime, "__final__", False)):
    ...
    def apply_decorator_to_funcitem(decorator: nodes.Expression, func: nodes.FuncItem) -> nodes.FuncItem | None:
        ...
        if decorator.fullname == "builtins.classmethod":
            if func.arguments[0].variable.name not in ("cls", "mcs", "metacls"):
                raise StubtestFailure(f"unexpected class argument name {func.arguments[0].variable.name!r} " f"in {dec.fullname}")
    ...
    try:
        try:
            ...
            if hasattr(runtime, "__text_signature__") and "<unrepresentable>" in runtime.__text_signature__:
                ...
                new_params = [
                    (parameter.replace(default=UNREPRESENTABLE) if parameter.default is ... else parameter) for parameter in sig.parameters.values()
                ]
    ...
    for module in modules:
        ...
        if not find_submodules:
            ...
            try:
                ...
                all_modules.extend(m.name for m in pkgutil.walk_packages(runtime.__path__, runtime.__name__ + ".") if m.name not in all_modules)
            except KeyboardInterrupt:
                raise
...
def test_stubs(args: _Arguments, use_builtins_fixtures: bool = False) -> int:
    ...
    if args.generate_allowlist:
        ...
        if error_count:
            print(
                _style(
                    f"Found {error_count} error{plural_s(error_count)}" f" (checked {len(modules)} module{plural_s(modules)})",
                    color="red",
                    bold=True,
                )
            )
