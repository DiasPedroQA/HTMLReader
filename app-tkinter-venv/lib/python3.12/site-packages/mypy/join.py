...
class TypeJoinVisitor(TypeVisitor[ProperType]):
    ...
    def visit_tuple_type(self, t: TupleType) -> ProperType:
        ...
        if isinstance(self.s, TupleType):
            if self.instance_joiner is None:
                self.instance_joiner = InstanceJoiner()
            fallback = self.instance_joiner.join_instances(mypy.typeops.tuple_fallback(self.s), mypy.typeops.tuple_fallback(t))
    ...
    def visit_typeddict_type(self, t: TypedDictType) -> ProperType:
        if isinstance(self.s, TypedDictType):
            items = {
                ...
                if (is_equivalent(s_item_type, t_item_type) and (item_name in t.required_keys) == (item_name in self.s.required_keys))
            }
...
def is_similar_params(t: Parameters, s: Parameters) -> bool:
    ...
    return len(t.arg_types) == len(s.arg_types) and t.min_args == s.min_args and (t.var_arg() is not None) == (s.var_arg() is not None)
