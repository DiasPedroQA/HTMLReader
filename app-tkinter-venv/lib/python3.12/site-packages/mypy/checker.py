...
class TypeChecker(NodeVisitor[None], TypeCheckerSharedApi):
    ...
    def __init__(
        ...
        self._expr_checker = mypy.checkexpr.ExpressionChecker(self, self.msg, self.plugin, per_line_checking_time_ns)
        ...
        *,
        ...
        with self.binder.frame_context(can_skip=False, conditional_frame=True):
            ...
            while True:
                ...
                # complex types.
                if (partials_new == partials_old) and (not self.binder.last_pop_changed or iter > 3) and (widened_new == widened_old or iter > 1):
                    break
    ...
    def check_overlapping_overloads(self, defn: OverloadedFuncDef) -> None:
        ...
        for i, item in enumerate(defn.items):
            ...
            if impl_type is not None:
                ...
                if not is_callable_compatible(impl, sig1, is_compat=is_subtype, is_proper_subtype=False, ignore_return=True):
    ...
    def get_generator_yield_type(self, return_type: Type, is_coroutine: bool) -> Type:
        ...
        if isinstance(return_type, AnyType):
            ...
            return make_simplified_union([self.get_generator_yield_type(item, is_coroutine) for item in return_type.items])
        elif not self.is_generator_return_type(return_type, is_coroutine) and not self.is_async_generator_return_type(return_type):
    ...
    def get_generator_receive_type(self, return_type: Type, is_coroutine: bool) -> Type:
        ...
        if isinstance(return_type, AnyType):
            ...
            return make_simplified_union([self.get_generator_receive_type(item, is_coroutine) for item in return_type.items])
        elif not self.is_generator_return_type(return_type, is_coroutine) and not self.is_async_generator_return_type(return_type):
            # If the function doesn't have a proper Generator (or
            # Awaitable) return type, anything is permissible.
            return AnyType(TypeOfAny.from_error)
            ...
            return AnyType(TypeOfAny.special_form)
        elif return_type.type.fullname in ("typing.Generator", "typing.AwaitableGenerator") and len(return_type.args) >= 3:
    ...
    def get_generator_return_type(self, return_type: Type, is_coroutine: bool) -> Type:
        ...
        if isinstance(return_type, AnyType):
            ...
            return make_simplified_union([self.get_generator_return_type(item, is_coroutine) for item in return_type.items])
        ...
        for item, typ in expanded:
            ...
            if not unreachable:
                ...
                show_error = (
                    ...
                    (not allow_empty and not (isinstance(defn, FuncDef) and defn.abstract_status != NOT_ABSTRACT) and not self.is_stub)
                )

                # Ignore plugin generated methods, these usually don't need any bodies.
                if defn.info is not FUNC_NO_INFO and (defn.name not in defn.info.names or defn.info.names[defn.name].plugin_generated):
                    show_error = False
                ...
                if self.options.warn_no_return:
                    if not self.current_node_deferred and not isinstance(return_type, (NoneType, AnyType)) and show_error:
        ...
        if isinstance(typ.ret_type, TypeVarType) and typ.ret_type in typ.variables:
            ...
            if typ.ret_type not in arg_type_visitor.arg_types:
                ...
                if not (isinstance(upper_bound, Instance) and upper_bound.type.fullname == "builtins.object"):
                    self.note(
                        "Consider using the upper bound " f"{format_type(typ.ret_type.upper_bound, self.options)} instead",
    ...
    def check_default_args(self, item: FuncItem, body_is_trivial: bool) -> None:
        for arg in item.arguments:
            ...
            if name.startswith("__tuple_arg_"):
                ...
                msg += f'argument "{name}"'
            if not self.options.implicit_optional and isinstance(arg.initializer, NameExpr) and arg.initializer.fullname == "builtins.None":
                notes = [
    ...
    def check_for_missing_annotations(self, fdef: FuncItem) -> None:
        ...
        if show_untyped and (self.options.disallow_untyped_defs or check_incomplete_defs):
            if fdef.type is None and self.options.disallow_untyped_defs:
                if not fdef.arguments or (len(fdef.arguments) == 1 and (fdef.arg_names[0] == "self" or fdef.arg_names[0] == "cls")):
    ...
    def check_reverse_op_method(self, defn: FuncItem, reverse_type: CallableType, reverse_name: str, context: Context) -> None:
        ...
        def has_readable_member(typ: UnionType | Instance, name: str) -> bool:
            ...
            return all(
                (isinstance(x, UnionType) and has_readable_member(x, name)) or (isinstance(x, Instance) and x.type.has_readable_member(name))
        ...
        for forward_item in flatten_nested_unions([forward_type]):
            ...
            if isinstance(forward_item, CallableType):
                if self.is_unsafe_overlapping_op(forward_item, forward_base, reverse_type):
                    self.msg.operator_method_signatures_overlap(reverse_class, reverse_name, forward_base, forward_name, context)
    ...
    def check_explicit_override_decorator(
        ...
        if (
            ...
            self.msg.explicit_override_decorator_missing(defn.name, found_method_base_classes[0].fullname, context or defn)
    ...
    def check_method_override_for_base_with_name(self, defn: FuncDef | OverloadedFuncDef | Decorator, name: str, base: TypeInfo) -> bool:
        ...
        if is_settable_property(defn) and (is_settable_property(original_node) or isinstance(original_node, Var)):
        ...
        if isinstance(original_type, AnyType) or isinstance(typ, AnyType):
            ...
            if (
                ...
                msg = message_registry.COVARIANT_OVERRIDE_OF_MUTABLE_ATTRIBUTE.with_additional_msg(
                    f' (base class "{base.name}" defined the type as {base_str},' f" override has type {override_str})"
                )
                self.fail(msg, context)
        elif isinstance(original_type, UnionType) and any(is_subtype(typ, orig_typ, ignore_pos_arg_names=True) for orig_typ in original_type.items):
            ...
            # covariant overrides.
            pass
        elif original_node and (not self.is_writable_attribute(original_node) or always_allow_covariant) and is_subtype(typ, original_type):
            # If the attribute is read-only, allow covariance
            pass
        else:
            self.msg.signature_incompatible_with_supertype(defn.name, name, base.name, context, original=original_type, override=typ)
        return False
        ...
        if fail:
            ...
            if (
                ...
                if not is_subtype(erase_override(override.ret_type), original.ret_type):
                    self.msg.return_type_incompatible_with_supertype(name, name_in_super, supertype, original.ret_type, override.ret_type, node)
                    emitted_msg = True
            ...
            if not emitted_msg:
                ...
                self.msg.signature_incompatible_with_supertype(name, name_in_super, supertype, node, original=original, override=override)
    ...
    def check_enum_new(self, defn: ClassDef) -> None:
        ...
        for base in defn.info.bases:
            ...
            if candidate and has_new:
                self.fail(
                    "Only a single data type mixin is allowed for Enum subtypes, " 'found extra "{}"'.format(base.str_with_options(self.options)),
                    defn,
                )
            elif candidate:
                has_new = True
    ...
    def check_protocol_variance(self, defn: ClassDef) -> None:
        ...
        for i, tvar in enumerate(tvars):
            if not isinstance(tvar, TypeVarType):
                ...
                continue
            up_args: list[Type] = [object_type if i == j else AnyType(TypeOfAny.special_form) for j, _ in enumerate(tvars)]
            down_args: list[Type] = [UninhabitedType() if i == j else AnyType(TypeOfAny.special_form) for j, _ in enumerate(tvars)]
        ...
        if isinstance(p_first_type, FunctionLike) and isinstance(p_second_type, FunctionLike):
            ...
            if ok:
                if (
                    ...
                    self.msg.fail(
                        f'Cannot override writeable attribute "{name}" in base "{base2.name}"' f' with read-only property in base "{base1.name}"',
                        ctx,
                        code=codes.OVERRIDE,
                    )
        else:
    ...
    def check_metaclass_compatibility(self, typ: TypeInfo) -> None:
        ...
        metaclasses = [
            entry.metaclass_type for entry in typ.mro[1:-1] if entry.metaclass_type and not is_named_instance(entry.metaclass_type, "builtins.type")
        ]
        if not metaclasses:
            return
        ...
        if isinstance(s, AssertStmt) and is_false_literal(s.expr):
            ...
            if isinstance(s.expr, EllipsisExpr):
                ...
                with self.expr_checker.msg.filter_errors():
                    typ = get_proper_type(self.expr_checker.accept(s.expr, allow_none_return=True, always_allow_any=True))
        ...
        if isinstance(lvalue, (TupleExpr, ListExpr)):
            ...
            if lvalue_type:
                if isinstance(lvalue_type, PartialType) and lvalue_type.type is None:
                    ...
                    rvalue_type, lvalue_type = self.check_simple_assignment(lvalue_type, rvalue, context=rvalue, inferred=inferred, lvalue=lvalue)
            ...
            if inferred:
                ...
                if not (inferred.is_final or inferred.is_index_var or (isinstance(lvalue, NameExpr) and lvalue.name == "__match_args__")):
                    rvalue_type = remove_instance_last_known_values(rvalue_type)
                self.infer_variable_type(inferred, lvalue, rvalue_type, rvalue)
            self.check_assignment_to_slots(lvalue)
            if isinstance(lvalue, RefExpr) and not (isinstance(lvalue, NameExpr) and lvalue.name == "__match_args__"):
        ...
        if inferred.info:
            for base in inferred.info.mro[1:]:
                ...
                base_type, _ = self.node_type_from_base(
                    ...
                    base,
                    inferred,
                    is_class=is_method(base_node) or isinstance(base_node, Var) and not is_instance_var(base_node),
                )
                if base_type and not (isinstance(base_node, Var) and base_node.invalid_partial_type) and not isinstance(base_type, PartialType):
    ...
    def check_compatibility_all_supers(self, lvalue: RefExpr, rvalue: Expression) -> None:
        ...
        if (
            ...
            and (lvalue_node.line == lvalue.line or lvalue_node.is_inferred and not lvalue_node.explicit_self_type)
            ...
            for base in lvalue_node.info.mro[1:]:
                ...
                if lvalue_node.allow_incompatible_override and not (lvalue_node.name == "__slots__" and base.fullname == "builtins.object"):
                    continue
                ...
                if base_type:
                    ...
                    if lvalue_type and custom_setter:
                        base_type, _ = self.node_type_from_base(lvalue_node.name, base, lvalue, setter_type=True)
                        # Setter type for a custom property must be ready if
                        # the getter type is ready.
                        assert base_type is not None
                        if not is_subtype(base_type, lvalue_type):
                            self.msg.incompatible_setter_override(lvalue, lvalue_type, base_type, base)
                            return
    ...
    def check_compatibility_super(
        ...
        if ok and codes.MUTABLE_OVERRIDE in self.options.enabled_error_codes and self.is_writable_attribute(base_node) and not always_allow_covariant:
            ok = self.check_subtype(
                base_type,
                compare_type,
                rvalue,
        ...
        *,
        ...
        with self.msg.filter_errors(filter_deprecated=True):
            if is_class:
                ...
                base_type = analyze_class_attribute_access(instance, name, mx, mcs_fallback=fallback, override_info=base)
            else:
    ...
    def check_assignment_to_multiple_lvalues(
        ...
        if isinstance(rvalue, (TupleExpr, ListExpr)):
            ...
            if iterable_start is not None and iterable_end is not None and iterable_type is not None:
                ...
                if rvalue_needed > 0:
                    rvalues = rvalues[0:iterable_start] + [TempNode(iterable_type) for i in range(rvalue_needed)] + rvalues[iterable_end + 1 :]
    ...
    def check_multi_assignment(
        ...
        if isinstance(rvalue_type, AnyType):
            ...
            self.check_multi_assignment_from_tuple(lvalues, rvalue, rvalue_type, context, undefined_rvalue, infer_lvalue_type)
    ...
    def check_multi_assignment_from_tuple(
        ...
        if self.check_rvalue_count_in_assignment(lvalues, len(rvalue_type.items), context, rvalue_unpack=rvalue_unpack):
            ...
            if not undefined_rvalue:
                ...
                if isinstance(reinferred_rvalue_type, UnionType):
                    self.check_multi_assignment_from_union(lvalues, rvalue, reinferred_rvalue_type, context, infer_lvalue_type)
                    return
            ...
            left_rv_types, star_rv_types, right_rv_types = self.split_around_star(rvalue_type.items, star_index, len(lvalues))
            ...
            if star_lv:
                list_expr = ListExpr(
                    [
                        (self.temp_node(rv_type, context) if not isinstance(rv_type, UnpackType) else StarExpr(self.temp_node(rv_type.type, context)))
            ...
            if infer_lvalue_type:
                for lv in lvalues:
                    if isinstance(lv, NameExpr) and isinstance(lv.node, Var) and lv.node.type is None:
                        ...
                        if isinstance(lv.expr, NameExpr) and isinstance(lv.expr.node, Var) and lv.expr.node.type is None:
                            lv.expr.node.type = self.named_generic_type("builtins.list", [AnyType(TypeOfAny.from_error)])
    ...
    def check_multi_assignment_from_iterable(
        ...
        rvalue_type = get_proper_type(rvalue_type)
        if self.type_is_iterable(rvalue_type) and isinstance(rvalue_type, (Instance, CallableType, TypeType, Overloaded)):
    ...
    def check_lvalue(self, lvalue: Lvalue, rvalue: Expression | None = None) -> tuple[Type | None, IndexExpr | None, Var | None]:
        lvalue_type = None
        index_lvalue = None
        inferred = None
        ...
        if self.is_definition(lvalue) and (not isinstance(lvalue, NameExpr) or isinstance(lvalue.node, Var)):
            ...
            if self.options.allow_redefinition_new and isinstance(lvalue.node, Var) and lvalue.node.is_inferred:
                inferred = lvalue.node
            self.store_type(lvalue, lvalue_type)
    ...
    def infer_partial_type(self, name: Var, lvalue: Lvalue, init_type: Type) -> bool:
        ...
        if isinstance(init_type, NoneType) and (isinstance(lvalue, MemberExpr) or not self.options.allow_redefinition_new):
            ...
            if (
                is_ref
                and (
                    fullname == "builtins.list" or fullname == "builtins.set" or fullname == "builtins.dict" or fullname == "collections.OrderedDict"
                )
        ...
        *,
        ...
        if self.is_stub and isinstance(rvalue, EllipsisExpr):
            ...
            if inferred is None or is_typeddict_type_context(lvalue_type):
                ...
                type_context = None
            rvalue_type = self.expr_checker.accept(rvalue, type_context=type_context, always_allow_any=always_allow_any)
            if lvalue_type is not None and type_context is None and not is_valid_inferred_type(rvalue_type, self.options):
                # Inference in an empty type context didn't produce a valid type, so
                # try using lvalue type as context instead.
                rvalue_type = self.expr_checker.accept(rvalue, type_context=lvalue_type, always_allow_any=always_allow_any)
            ...
            if isinstance(lvalue, NameExpr) and inferred is not None and inferred.type is not None and not inferred.is_final:
    ...
    def check_member_assignment(
        ...
        if (isinstance(instance_type, FunctionLike) and instance_type.is_type_obj()) or isinstance(instance_type, TypeType):
            rvalue_type, _ = self.check_simple_assignment(attribute_type, rvalue, context)
            return rvalue_type, attribute_type, True

        with self.msg.filter_errors(filter_deprecated=True):
            get_lvalue_type = self.expr_checker.analyze_ordinary_member_access(lvalue, is_lvalue=False)
        ...
        # what a user expects.
        rvalue_type, _ = self.check_simple_assignment(attribute_type, rvalue, context)
        infer = is_subtype(rvalue_type, get_lvalue_type) and is_subtype(get_lvalue_type, attribute_type)
    ...
    def try_infer_partial_type_from_indexed_assignment(self, lvalue: IndexExpr, rvalue: Expression) -> None:
        ...
        if isinstance(var, Var):
            if isinstance(var.type, PartialType):
                ...
                if typename == "builtins.dict" or typename == "collections.OrderedDict" or typename == "collections.defaultdict":
    ...
    def check_return_stmt(self, s: ReturnStmt) -> None:
        ...
        if defn is not None:
            ...
            if s.expr:
                ...
                # Return with a value.
                typ = get_proper_type(self.expr_checker.accept(s.expr, return_type, allow_none_return=allow_none_func_call))
    ...
    def visit_operator_assignment_stmt(self, s: OperatorAssignmentStmt) -> None:
        ...
        if inplace:
            ...
            if isinstance(inst := get_proper_type(lvalue_type), Instance) and isinstance(defn := inst.type.get_method(method), OverloadedFuncDef):
                for item in defn.items:
                    if isinstance(item, Decorator) and isinstance(typ := item.func.type, CallableType) and (bind_self(typ) == method_type):
    ...
    def get_types_from_except_handler(self, typ: Type, n: Expression) -> list[Type]:
        ...
        if isinstance(typ, TupleType):
            ...
            return [union_typ for item in typ.relevant_items() for union_typ in self.get_types_from_except_handler(item, n)]
    ...
    def visit_for_stmt(self, s: ForStmt) -> None:
        ...
        self.accept_loop(
            s.body,
            s.else_body,
            on_enter_body=lambda: self.analyze_index_variables(s.index, item_type, s.index_type is None, s),
        )
    ...
    def analyze_iterable_item_type(self, expr: Expression) -> tuple[Type, Type]:
        ...
        iterator, iterable = self.analyze_iterable_item_type_without_expression(self.expr_checker.accept(expr), context=expr)
    ...
    def visit_decorator_inner(self, e: Decorator, allow_empty: bool = False, skip_first_item: bool = False) -> None:
        ...
        for d in reversed(e.decorators[1:] if skip_first_item else e.decorators):
            ...
            sig, t2 = self.expr_checker.check_call(dec, [temp], [nodes.ARG_POS], e, callable_name=fullname, object_type=object_type)
    ...
    def visit_with_stmt(self, s: WithStmt) -> None:
        ...
        for expr, target in zip(s.expr, s.target):
            ...
            if is_literal_type(exit_ret_type, "builtins.bool", True) or (
                isinstance(exit_ret_type, Instance) and exit_ret_type.type.fullname == "builtins.bool" and state.strict_optional
            ):
            ...
            for p, g, b in zip(s.patterns, s.guards, s.bodies):
                ...
                with self.binder.frame_context(can_skip=True, fall_through=2):
                    if b.is_unreachable or isinstance(get_proper_type(pattern_type.type), UninhabitedType):
                        ...
                        pattern_map, else_map = conditional_types_to_typemaps(named_subject, pattern_type.type, pattern_type.rest_type)
                                ...
                                if isinstance(case_target, NameExpr):
                                    for type_map in (guard_map, else_map):
                                        ...
                                        for expr in list(type_map):
                                            if not (isinstance(expr, NameExpr) and expr.fullname == case_target.fullname):
                                                continue
    ...
    def intersect_instances(self, instances: tuple[Instance, Instance], errors: list[tuple[str, str]]) -> Instance | None:
        ...
        def _make_fake_typeinfo_and_full_name(base_classes_: list[Instance], curr_module_: MypyFile, options: Options) -> tuple[TypeInfo, str]:
    ...
    def check_for_truthy_type(self, t: Type, expr: Expression) -> None:
        ...
        if isinstance(t, FunctionLike):
            ...
            self.fail(
                message_registry.ITERABLE_ALWAYS_TRUE.format(format_expr_type(), format_type(Instance(info, t.args), self.options)),
                expr,
            )
        else:
    ...
    def find_type_equals_check(self, node: ComparisonExpr, expr_indices: list[int]) -> tuple[TypeMap, TypeMap]:
        ...
        for expr in exprs_in_type_calls:
            current_if_type, current_else_type = self.conditional_types_with_intersection(self.lookup_type(expr), type_being_compared, expr)
    ...
    def find_isinstance_check_helper(self, node: Expression, *, in_boolean_context: bool = True) -> tuple[TypeMap, TypeMap]:
            ...
            if refers_to_fullname(node.callee, "builtins.isinstance"):
                ...
                if literal(expr) == LITERAL_TYPE:
                    return conditional_types_to_typemaps(
                        expr,
                        *self.conditional_types_with_intersection(self.lookup_type(expr), self.get_isinstance_type(node.args[1]), expr),
                    )
    ...
    def comparison_type_narrowing_helper(self, node: ComparisonExpr) -> tuple[TypeMap, TypeMap]:
        ...
        simplified_operator_list = group_comparison_operands(node.pairwise(), narrowable_operand_index_to_hash, {"==", "is"})
    ...
    def equality_type_narrowing_helper(
        ...
        if operator in {"is", "is not"}:
            ...
            def has_no_custom_eq_checks(t: Type) -> bool:
                return not custom_special_method(t, "__eq__", check_all=False) and not custom_special_method(t, "__ne__", check_all=False)

            is_valid_target = is_exactly_literal_type
            coerce_only_in_literal_context = True

            expr_types = [operand_types[i] for i in expr_indices]
            should_narrow_by_identity = all(map(has_no_custom_eq_checks, expr_types)) and not is_ambiguous_mix_of_enums(expr_types)
        ...
        if if_map == {} and else_map == {}:
            if_map, else_map = self.refine_away_none_in_comparison(operands, operand_types, expr_indices, narrowable_operand_index_to_hash.keys())
    ...
    def refine_instance_type_with_len(self, typ: Instance, op: str, size: int) -> tuple[Type | None, Type | None]:
        ...
        allow_precise = (PRECISE_TUPLE_TYPES in self.options.enable_incomplete_feature) and typ.type.fullname == "builtins.tuple"
        if op in ("==", "is"):
            # TODO: return fixed union + prefixed variadic tuple for no_type?
    ...
    def check_subtype(
        ...
        *,
        ...
        if self.msg.try_report_long_tuple_assignment_error(subtype, supertype, context, msg, subtype_label, supertype_label):
            return False
        extra_info: list[str] = []
        note_msg = ""
        notes = notes or []
    ...
    def check_possible_missing_await(self, subtype: Type, supertype: Type, context: Context, code: ErrorCode | None) -> None:
        ...
        if "." not in name:
            ...
            if last in n.names:
                ...
                if fullname in SUGGESTED_TEST_FIXTURES:
                    suggestion = ", e.g. add '[{} fixtures/{}]' to your test".format(parts[0], SUGGESTED_TEST_FIXTURES[fullname])
                else:
                    suggestion = ""
                raise KeyError(
    ...
    def enter_partial_types(self, *, is_function: bool = False, is_class: bool = False) -> Iterator[None]:
        ...
        if not self.current_node_deferred:
            for var, context in partial_types.items():
                ...
                allow_none = not self.options.local_partial_types or is_function or (is_class and self.is_defined_in_base_class(var))
    ...
    def add_any_attribute_to_type(self, typ: Type, name: str) -> Type:
        ...
        if isinstance(typ, UnionType):
            ...
            return make_simplified_union(with_attr + [self.add_any_attribute_to_type(typ, name) for typ in without_attr])
        return orig_typ
    ...
    def warn_deprecated(self, node: Node | None, context: Context) -> None:
        ...
        if (
            ...
            and not any(node.fullname == p or node.fullname.startswith(f"{p}.") for p in self.options.deprecated_calls_exclude)
        ):
            warn = self.msg.note if self.options.report_deprecated_as_note else self.msg.fail
            warn(deprecated, context, code=codes.DEPRECATED)

    def warn_deprecated_overload_item(self, node: Node | None, context: Context, *, target: Type, selftype: Type | None = None) -> None:
...
def conditional_types(
    ...
    if proposed_type_ranges:
        ...
        if isinstance(proposed_type, AnyType):
            ...
            proposed_precise_type = UnionType.make_union([type_range.item for type_range in proposed_type_ranges if not type_range.is_upper_bound])
...
def expand_callable_variants(c: CallableType) -> list[CallableType]:
    ...
    for tv in c.variables:
        ...
        if tv.id.is_self():
            c = expand_type(c, {tv.id: tv.upper_bound}).copy_modified(variables=[v for v in c.variables if not v.id.is_self()])
            break
...
def overload_can_never_match(signature: CallableType, other: CallableType) -> bool:
    ...
    return is_callable_compatible(exp_signature, other, is_compat=is_more_precise, is_proper_subtype=True, ignore_return=True)
...
def is_valid_inferred_type(typ: Type, options: Options, is_lvalue_final: bool = False, is_lvalue_member: bool = False) -> bool:
