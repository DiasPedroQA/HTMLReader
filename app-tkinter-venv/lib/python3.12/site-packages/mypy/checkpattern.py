...
class PatternChecker(PatternVisitor[PatternType]):
    ...
    def visit_value_pattern(self, o: ValuePattern) -> PatternType:
        ...
        narrowed_type, rest_type = self.chk.conditional_types_with_intersection(current_type, [get_type_range(typ)], o, default=get_proper_type(typ))
    ...
    def narrow_sequence_child(self, outer_type: Type, inner_type: Type, ctx: Context) -> Type:
        ...
        if is_subtype(new_type, outer_type):
            new_type, _ = self.chk.conditional_types_with_intersection(outer_type, [get_type_range(new_type)], ctx, default=outer_type)
        else:
            new_type = outer_type
        return new_type
