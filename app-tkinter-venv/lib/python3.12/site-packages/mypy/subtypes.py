...
def is_proper_subtype(
    ...
    *,
    ...
    if subtype_context is None:
        ...
        assert not any({ignore_promotions, erase_instances, keep_erased_types}), "Don't pass both context and individual flags"
...
def is_same_type(a: Type, b: Type, ignore_promotions: bool = True, subtype_context: SubtypeContext | None = None) -> bool:
    ...
    # from plain int, etc.
    return is_proper_subtype(a, b, ignore_promotions=ignore_promotions, subtype_context=subtype_context) and is_proper_subtype(
...
def _is_subtype(left: Type, right: Type, subtype_context: SubtypeContext, proper_subtype: bool) -> bool:
    ...
    if not proper_subtype and isinstance(right, (AnyType, UnboundType, ErasedType)) and not isinstance(left, UnpackType):
    ...
    if isinstance(right, UnionType) and not isinstance(left, UnionType):
        ...
        if proper_subtype:
            is_subtype_of_item = any(is_proper_subtype(orig_left, item, subtype_context=subtype_context) for item in right.items)
        else:
            is_subtype_of_item = any(is_subtype(orig_left, item, subtype_context=subtype_context) for item in right.items)
        ...
        if not is_subtype_of_item and isinstance(left, Instance) and (left.type.is_enum or left.type.fullname == "builtins.bool"):
...
class SubtypeVisitor(TypeVisitor[bool]):
    ...
    def visit_instance(self, left: Instance) -> bool:
        ...
        if isinstance(right, Instance):
            ...
            if (
                ...
                if not self.subtype_context.ignore_type_params:
                    ...
                    for lefta, righta, tvar in type_params:
                        if isinstance(tvar, TypeVarType):
                            ...
                            if not check_type_parameter(lefta, righta, variance, self.proper_subtype, self.subtype_context):
                                nominal = False
                        else:
                            # TODO: everywhere else ParamSpecs are handled as invariant.
                            if not check_type_parameter(lefta, righta, COVARIANT, self.proper_subtype, self.subtype_context):
                                nominal = False
                if nominal:
                    type_state.record_subtype_cache_entry(self._subtype_kind, left, right)
                else:
                ...
                return nominal
            if right.type.is_protocol and is_protocol_implementation(left, right, proper_subtype=self.proper_subtype, options=self.options):
                return True
    ...
    def visit_callable_type(self, left: CallableType) -> bool:
        ...
        if isinstance(right, CallableType):
            if left.type_guard is not None and right.type_guard is not None:
                ...
                # *not* a Parent.
                if not self._is_subtype(left.type_is, right.type_is) or not self._is_subtype(right.type_is, left.type_is):
                    return False
            ...
            return is_callable_compatible(
                ...
                ignore_pos_arg_names=self.subtype_context.ignore_pos_arg_names,
                strict_concatenate=((self.options.extra_checks or self.options.strict_concatenate) if self.options else False),
            )
            ...
            if right.type.is_protocol and left.is_type_obj():
                ...
                if isinstance(ret_type, Instance) and is_protocol_implementation(ret_type, right, proper_subtype=self.proper_subtype, class_obj=True):
                    return True
            ...
            if is_named_instance(right, "typing.Sized"):
                ...
                for li in left.items:
                    ...
                    if not self._is_subtype(li, iter_type):
                        return False
                return True
            elif self._is_subtype(left.partial_fallback, right) and self._is_subtype(mypy.typeops.tuple_fallback(left), right):
                return True
            return False
    ...
    def variadic_tuple_subtype(self, left: TupleType, right: TupleType) -> bool:
        ...
        if left_unpack_index is None:
            ...
            if not all(self._is_subtype(li, ri) for li, ri in zip(prefix, right.items[:right_prefix])):
                return False
            if right_suffix and not all(self._is_subtype(li, ri) for li, ri in zip(suffix, right.items[-right_suffix:])):
                return False
            ...
            if not isinstance(left_unpacked, Instance):
                ...
                if self.is_top_type(right_item):
                    right_prefix_types, middle, right_suffix_types = split_with_prefix_and_suffix(tuple(right.items), left_prefix, left_suffix)
            ...
            for right_item in right.items:
                ...
                for left_index, left_item in enumerate(left.items):
                    ...
                    if subtype_match and previous_match_left_index <= left_index:
                        ...
                        strict_concat = (self.options.extra_checks or self.options.strict_concatenate) if self.options else False
    ...
    def visit_type_type(self, left: TypeType) -> bool:
        ...
        if isinstance(right, Instance):
            ...
            if isinstance(item, Instance):
                if right.type.is_protocol and is_protocol_implementation(item, right, proper_subtype=self.proper_subtype, class_obj=True):
                    return True
...
def find_node_type(
    ...
    if isinstance(node, FuncBase) or (isinstance(p_typ, FunctionLike) and node.is_initialized_in_class and not node.is_staticmethod):
