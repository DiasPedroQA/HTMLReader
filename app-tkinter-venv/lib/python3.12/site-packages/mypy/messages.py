...
class MessageBuilder:
            ...
            if not self.are_type_names_disabled():
                ...
                if isinstance(original_type, Instance) and original_type.type.names:
                    if module_symbol_table is not None and member in module_symbol_table and not module_symbol_table[member].module_public:
                        self.fail(
                            f"{format_type(original_type, self.options, module_names=True)} does not " f'explicitly export attribute "{member}"',
                            context,
    ...
    def incompatible_argument(
        ...
        if callee_name is not None:
            ...
            if name.startswith('"__setitem__" of'):
                if n == 1:
                    ...
                    arg_type_str, callee_type_str = format_type_distinctly(arg_type, callee.arg_types[n - 1], options=self.options)
        ...
        if callee_name == "<list>":
            ...
            n -= 1
            actual_type_str, expected_type_str = format_type_distinctly(arg_type, callee.arg_types[0], options=self.options)
            msg = "{} item {} has incompatible type {}; expected {}".format(name.title(), n, actual_type_str, expected_type_str)
            ...
            if is_subtype(key_type, expected_key_type):
                ...
                key_type_str, expected_key_type_str = format_type_distinctly(key_type, expected_key_type, options=self.options)
            if is_subtype(value_type, expected_value_type):
                ...
                value_type_str, expected_value_type_str = format_type_distinctly(value_type, expected_value_type, options=self.options)
            ...
            value_type_str, expected_value_type_str = format_type_distinctly(arg_type, callee.arg_types[n - 1], options=self.options)
            msg = "Unpacked dict entry {} has incompatible type {}; expected {}".format(n - 1, value_type_str, expected_value_type_str)
            code = codes.DICT_ITEM
            ...
            msg = "List comprehension has incompatible type List[{}]; expected List[{}]".format(actual_type_str, expected_type_str)
        elif callee_name == "<set-comprehension>":
            actual_type_str, expected_type_str = map(
                strip_quotes,
                format_type_distinctly(arg_type, callee.arg_types[0], options=self.options),
            )
            msg = "Set comprehension has incompatible type Set[{}]; expected Set[{}]".format(actual_type_str, expected_type_str)
        elif callee_name == "<dictionary-comprehension>":
            actual_type_str, expected_type_str = format_type_distinctly(arg_type, callee.arg_types[n - 1], options=self.options)
            ...
            if self.prefer_simple_messages():
                ...
                arg_type_str, expected_type_str = format_type_distinctly(arg_type, expected_type, bare=True, options=self.options)
                ...
                if (
                    ...
                    arg_type_str, expected_type_str = format_type_distinctly(arg_type.items[arg_name], expected_type, bare=True, options=self.options)
                    arg_label = f'"{arg_name}"'
    ...
    def maybe_note_about_special_args(self, callee: CallableType, context: Context) -> None:
        ...
        if first_arg and first_arg not in {"self", "cls", "mcs"}:
            self.note(
                "Looks like the first special argument in a method " 'is not named "self", "cls", or "mcs", ' "maybe it is missing?",
                context,
            )

    def unexpected_keyword_argument_for_function(self, for_func: str, name: str, context: Context, *, matches: list[str] | None = None) -> None:
    ...
    def duplicate_argument_value(self, callee: CallableType, index: int, context: Context) -> None:
        self.fail(
            '{} gets multiple values for keyword argument "{}"'.format(callable_name(callee) or "Function", callee.arg_names[index]),
            context,
        )
    ...
    def incompatible_setter_override(self, defn: Context, typ: Type, original_type: Type, base: TypeInfo) -> None:
    ...
    def return_type_incompatible_with_supertype(
        ...
        self.fail(
            'Return type {} of "{}" incompatible with return type {} in {}'.format(override_str, name, original_str, target),
            context,
            code=codes.OVERRIDE,
        )
    ...
    def incompatible_type_application(self, min_arg_count: int, max_arg_count: int, actual_arg_count: int, context: Context) -> None:
        if max_arg_count == 0:
    ...
    def cannot_instantiate_abstract_class(self, class_name: str, abstract_attributes: dict[str, bool], context: Context) -> None:
        ...
        self.fail(
            f'Cannot instantiate abstract class "{class_name}" with abstract ' f"attribute{plural_s(abstract_attributes)} {attrs}",
            context,
            code=codes.ABSTRACT,
        )
        attrs_with_none = [f'"{a}"' for a, implicit_and_can_return_none in abstract_attributes.items() if implicit_and_can_return_none]
    ...
    def base_class_definitions_incompatible(self, name: str, base1: TypeInfo, base2: TypeInfo, context: Context) -> None:
        self.fail(
            'Definition of "{}" in base class "{}" is incompatible ' 'with definition in base class "{}"'.format(name, base1.name, base2.name),
            context,
        )
    ...
    def incompatible_typevar_value(self, callee: CallableType, typ: Type, typevar_name: str, context: Context) -> None:
        self.fail(
            message_registry.INCOMPATIBLE_TYPEVAR_VALUE.format(typevar_name, callable_name(callee) or "function", format_type(typ, self.options)),
            context,
            code=codes.TYPE_VAR,
        )
    ...
    def unexpected_typeddict_keys(
        ...
        if not typ.is_anonymous():
            ...
            if missing:
                self.fail(
                    "Missing {} for TypedDict {}".format(format_key_list(missing, short=True), format_type(typ, self.options)),
                    context,
                    code=codes.TYPEDDICT_ITEM,
                )
            ...
            if extra:
                self.fail(
                    "Extra {} for TypedDict {}".format(format_key_list(extra, short=True), format_type(typ, self.options)),
                    context,
    ...
    def typeddict_key_must_be_string_literal(self, typ: TypedDictType, context: Context) -> None:
        self.fail(
            "TypedDict key must be a string literal; expected one of {}".format(format_item_name_list(typ.items.keys())),
            context,
    ...
    def typeddict_key_not_found(self, typ: TypedDictType, item_name: str, context: Context, setitem: bool = False) -> None:
        ...
        if typ.is_anonymous():
            self.fail(
                '"{}" is not a valid TypedDict key; expected one of {}'.format(item_name, format_item_name_list(typ.items.keys())),
                context,
            )
        else:
    ...
    def incorrect__exit__return(self, context: Context) -> None:
        ...
        self.note(
            'If return type of "__exit__" implies that it may return True, ' "the context manager may swallow exceptions",
            context,
            code=codes.EXIT_RETURN,
        )
    ...
    def bad_proto_variance(self, actual: int, tvar_name: str, expected: int, context: Context) -> None:
        msg = capitalize(
            '{} type variable "{}" used in protocol where {} one is expected'.format(variance_string(actual), tvar_name, variance_string(expected))
        )
        self.fail(msg, context)
    ...
    def tvar_without_default_type(self, tvar_name: str, last_tvar_name_with_default: str, context: Context) -> None:
        self.fail(
            f'"{tvar_name}" cannot appear after "{last_tvar_name_with_default}" ' "in type parameter list because it has no default type",
            context,
        )
    ...
    def report_protocol_problems(
        ...
        *,
        ...
        if missing and (len(missing) < len(supertype.type.protocol_members) or missing == ["__call__"]) and len(missing) <= MAX_ITEMS:
            if missing == ["__call__"] and class_obj:
                self.note(
                    '"{}" has constructor incompatible with "__call__" of "{}"'.format(subtype.type.name, supertype.type.name),
                    context,
                    code=code,
                )
            else:
                self.note(
                    '"{}" is missing following "{}" protocol member{}:'.format(subtype.type.name, supertype.type.name, plural_s(missing)),
                    context,
                    code=code,
                )
        ...
        for name, subflags, superflags in conflict_flags[:MAX_ITEMS]:
            if not class_obj and IS_CLASSVAR in subflags and IS_CLASSVAR not in superflags:
                self.note(
                    "Protocol member {}.{} expected instance variable, got class variable".format(supertype.type.name, name),
                    context,
                    code=code,
                )
            if not class_obj and IS_CLASSVAR in superflags and IS_CLASSVAR not in subflags:
                self.note(
                    "Protocol member {}.{} expected class variable, got instance variable".format(supertype.type.name, name),
                    context,
                    code=code,
                )
            if IS_SETTABLE in superflags and IS_SETTABLE not in subflags:
                self.note(
                    "Protocol member {}.{} expected settable variable," " got read-only attribute".format(supertype.type.name, name),
                    context,
                    code=code,
                )
            ...
            if class_obj and IS_CLASSVAR in superflags:
                self.note(
                    "ClassVar protocol member {}.{} can never be matched by a class object".format(supertype.type.name, name),
                    context,
                    code=code,
                )
    ...
    def try_report_long_tuple_assignment_error(
        ...
        """
        if isinstance(subtype, TupleType):
            if len(subtype.items) > MAX_TUPLE_ITEMS and isinstance(supertype, Instance) and supertype.type.fullname == "builtins.tuple":
                ...
                return True
            elif isinstance(supertype, TupleType) and (len(subtype.items) > MAX_TUPLE_ITEMS or len(supertype.items) > MAX_TUPLE_ITEMS):
    ...
    def add_fixture_note(self, fullname: str, ctx: Context) -> None:
        ...
        if fullname in SUGGESTED_TEST_FIXTURES:
            self.note(
                "Consider adding [builtins fixtures/{}] to your test description".format(SUGGESTED_TEST_FIXTURES[fullname]),
                ctx,
            )
...
def format_type_inner(
    ...
    if isinstance(typ, Instance):
        ...
        if typ.prefix.arg_types:
            args = format_callable_args(typ.prefix.arg_types, typ.prefix.arg_kinds, typ.prefix.arg_names, format, verbosity)
        ...
        if len(literal_items) > 1:
            ...
            if print_as_optional:
                ...
                s = format_union(typ.items) if options.use_or_syntax() else f"Union[{', '.join(format_union_items(typ.items))}]"
            return s
...
def pretty_callable(tp: CallableType, options: Options, skip_self: bool = False) -> str:
    ...
    for i in range(len(tp.arg_types)):
        ...
        if (
            ...
            and name is None
            and (i == len(tp.arg_types) - 1 or (tp.arg_names[i + 1] is not None or not tp.arg_kinds[i + 1].is_positional()))
        ):
            s += ", /"
            slash = True
...
def append_invariance_notes(notes: list[str], arg_type: Instance, expected_type: Instance) -> list[str]:
    ...
    if invariant_type and covariant_suggestion:
        notes.append(f'"{invariant_type}" is invariant -- see ' + "https://mypy.readthedocs.io/en/stable/common_issues.html#variance")
