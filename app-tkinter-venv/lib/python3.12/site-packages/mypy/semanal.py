...
class SemanticAnalyzer(NodeVisitor[None], SemanticAnalyzerInterface, SemanticAnalyzerPluginInterface):
                ...
                if (
                    ...
                    and (
                        ...
                        or (
                            ...
                            and (self.is_stub_file or self.is_future_flag_set("annotations") or self.options.python_version >= (3, 9))
                        )
                    )
                    and typ.args
                ):
                ...
                for d in item.decorators:
                    if deprecation and refers_to_fullname(d, OVERLOAD_NAMES):
                        ...
                        if isinstance(typ := item.func.type, CallableType):
                            ...
                            typestr = " "
                        item.func.deprecated = f"overload{typestr}of function {defn.fullname} is deprecated: " f"{deprecated}"
                ...
                if item.decorators:
                    ...
                    if isinstance(first_node, MemberExpr):
                        if first_node.name == "setter":
                            ...
                            setter_func_type = function_type(item.func, self.named_type("builtins.function"))
    ...
    def push_type_args(self, type_args: list[TypeParam] | None, context: Context) -> list[tuple[str, TypeVarLikeExpr]] | None:
        if not type_args:
            return []
    ...
    def analyze_base_classes(self, base_type_exprs: list[Expression]) -> tuple[list[tuple[ProperType, Expression]], bool] | None:
        ...
        for base_expr in base_type_exprs:
            if (isinstance(base_expr, RefExpr) and base_expr.fullname in TYPED_NAMEDTUPLE_NAMES + TPDICT_NAMES) or (
                isinstance(base_expr, CallExpr) and isinstance(base_expr.callee, RefExpr) and base_expr.callee.fullname in TPDICT_NAMES
            ):
        ...
        while True:
            ...
            if not fname and isinstance(call.callee, MemberExpr):
                ...
                if isinstance(callee_expr, RefExpr) and callee_expr.fullname:
                    ...
                    isinstance(callee_expr, IndexExpr) and isinstance(callee_expr.base, RefExpr) and isinstance(callee_expr.analyzed, TypeApplication)
                ):
                    method_name = call.callee.name
    ...
    def analyze_alias(
        ...
        try:
            typ = expr_to_unanalyzed_type(rvalue, self.options, self.is_stub_file, lookup_qualified=self.lookup_qualified)
        except TypeTranslationError:
    ...
    def check_and_set_up_type_alias(self, s: AssignmentStmt) -> bool:
        ...
        if existing and (
            ...
            if isinstance(existing.node, TypeAlias) and not s.is_alias_def:
                self.fail(
                    'Cannot assign multiple types to name "{}"' ' without an explicit "Type[...]" annotation'.format(lvalue.name),
                    lvalue,
                )
            return False
        ...
        no_args = isinstance(res, ProperType) and isinstance(res, Instance) and not res.args and not empty_tuple_index and not pep_695
    ...
    def disable_invalid_recursive_aliases(self, s: AssignmentStmt | TypeAliasStmt, current_node: TypeAlias, ctx: Context) -> None:
                ...
                for lval in lvals:
                    ...
                    if lnode:
                        if isinstance(lnode.node, MypyFile) and lnode.node is not rnode.node:
                            ...
                            self.fail(
                                'Cannot assign multiple modules to name "{}" ' 'without explicit "types.ModuleType" annotation'.format(lval.name),
                                ctx,
                            )
    ...
    def visit_type_alias_stmt(self, s: TypeAliasStmt) -> None:
        ...
        try:
            ...
            if existing and not (
                isinstance(existing.node, TypeAlias) or (isinstance(existing.node, PlaceholderNode) and existing.node.line == s.line)
            ):
    ...
    def analyze_type_application(self, expr: IndexExpr) -> None:
        ...
        if isinstance(base, RefExpr) and isinstance(base.node, TypeAlias):
            ...
            if isinstance(target, Instance):
                ...
                if (
                    alias.no_args
                    and name in get_nongen_builtins(self.options.python_version)  # this avoids bogus errors for already reported aliases
    ...
    def is_active_symbol_in_class_body(self, node: SymbolNode | None) -> bool:
        ...
        if self.statement is None:
            ...
            return True
        if node is None or self.is_textually_before_statement(node) or not self.is_defined_in_current_module(node.fullname):
            return True
    ...
    def add_symbol(
        ...
        return self.add_symbol_table_node(name, symbol, context, can_defer, escape_comprehensions, no_progress, type_param)
    ...
    def name_not_defined(self, name: str, ctx: Context, namespace: str | None = None) -> None:
        ...
        for module in modules_with_unimported_hints:
            ...
            hint = ('Did you forget to import it from "{module}"?' ' (Suggestion: "from {module} import {name}")').format(
                module=module, name=lowercased[fullname].rsplit(".", 1)[-1]
            )
