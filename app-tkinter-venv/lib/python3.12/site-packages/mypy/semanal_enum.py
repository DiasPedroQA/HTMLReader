...
class EnumCallAnalyzer:
    ...
    def parse_enum_call_args(self, call: CallExpr, class_name: str) -> tuple[str, list[str], list[Expression | None], bool]:
        ...
        if isinstance(names, StrExpr):
            ...
            if is_StrExpr_list(seq_items):
                ...
                isinstance(seq_item, (TupleExpr, ListExpr)) and len(seq_item.items) == 2 and isinstance(seq_item.items[0], StrExpr)
            ...
            if proper_type is not None and isinstance(proper_type, LiteralType) and isinstance(proper_type.value, str):
                ...
                return self.fail_enum_call_arg(
                    "Second argument of %s() must be string, tuple, list or dict literal for mypy to determine Enum members" % class_name,
                    call,
                )
        else:
    ...
    def fail_enum_call_arg(self, message: str, context: Context) -> tuple[str, list[str], list[Expression | None], bool]:
