...
class SuggestionEngine:
    ...
    def filter_options(self, guesses: list[CallableType], is_method: bool, ignore_return: bool) -> list[CallableType]:
        ...
        return [t for t in guesses if self.flex_any is None or any_score_callable(t, is_method, ignore_return) >= self.flex_any]
        ...
        if ":" in key:
            if key.count(":") > 1:
                raise SuggestionFailure(
                    "Malformed location for function: {}. Must be either" " package.module.Class.method or path/to/file.py:line".format(key)
                )
            file, line = key.split(":")
            if not line.isdigit():
    ...
    def extract_from_decorator(self, node: Decorator) -> FuncDef | None:
        for dec in node.decorators:
            ...
            if not isinstance(typ, FunctionLike):
                return None
            for ct in typ.items:
                if not (len(ct.arg_types) == 1 and isinstance(ct.arg_types[0], TypeVarType) and ct.arg_types[0] == ct.ret_type):
                    return None
...
def refine_type(ti: Type, si: Type) -> Type:
    ...
    if isinstance(t, TupleType) and isinstance(s, TupleType) and t.partial_fallback == s.partial_fallback and len(t.items) == len(s.items):
