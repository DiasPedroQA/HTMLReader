...
def load_plugins_from_config(options: Options, errors: Errors, stdout: TextIO) -> tuple[list[Plugin], dict[str, str]]:
    ...
    for plugin_path in options.plugins:
        ...
        if not isinstance(plugin_type, type):
            plugin_error('Type object expected as the return value of "plugin"; got {!r} (in {})'.format(plugin_type, plugin_path))
        if not issubclass(plugin_type, Plugin):
            plugin_error('Return value of "plugin" must be a subclass of "mypy.plugin.Plugin" ' "(in {})".format(plugin_path))
        try:
...
class BuildManager:
    ...
    def __init__(
        ...
        self.cache_enabled = options.incremental and (not options.fine_grained_incremental or options.use_fine_grained_cache) and not has_reporters
        self.fscache = fscache
        self.find_module_cache = FindModuleCache(self.search_paths, self.fscache, self.options, source_set=self.source_set)
...
def find_cache_meta(id: str, path: str, manager: BuildManager) -> CacheMeta | None:
    ...
    plugin_data = json_loads(json_dumps(manager.plugin.report_config_data(ReportConfigContext(id, path, is_check=True))))
...
class State:
    ...
    def compute_dependencies(self) -> None:
        ...
        dep_entries = manager.all_imported_modules_in_file(self.tree) + self.manager.plugin.get_additional_deps(self.tree)
    ...
    def detect_possibly_undefined_vars(self) -> None:
        ...
        if manager.errors.is_error_code_enabled(codes.POSSIBLY_UNDEFINED) or manager.errors.is_error_code_enabled(codes.USED_BEFORE_DEF):
            self.tree.accept(
    ...
    def write_cache(self) -> None:
        ...
        assert len(set(self.dependencies)) == len(self.dependencies), f"Duplicates in dependencies list for {self.id} ({self.dependencies})"
...
def find_module_and_diagnose(
    ...
    if isinstance(result, str):
        ...
        if (
            ...
            or (result.endswith(".pyi") and not options.follow_imports_for_stubs)  # Stubs are always normal  # except when they aren't
...
def module_not_found(
    ...
    if target == "builtins":
        ...
        elif reason == ModuleNotFoundReason.FOUND_WITHOUT_TYPE_HINTS or reason == ModuleNotFoundReason.APPROVED_STUBS_NOT_INSTALLED:
                    ...
                    if newst.path:
                        ...
                        if newst_path in seen_files:
                            manager.errors.report(
                                -1,
                                0,
                                "Source file found twice under different module names: " '"{}" and "{}"'.format(seen_files[newst_path].id, newst.id),
                                blocker=True,
                            )
                            manager.errors.report(
                                -1,
...
def process_graph(graph: Graph, manager: BuildManager) -> None:
    ...
    for ascc in sccs:
        ...
        if manager.options.verbosity >= 2:
            for id in scc:
                manager.trace(
                    f"Priorities for {id}:",
                    " ".join("%s:%d" % (x, graph[id].priorities[x]) for x in graph[id].dependencies if x in ascc and x in graph[id].priorities),
                )
    ...
    if sccs_left:
        manager.log("{} fresh SCCs ({} nodes) left in queue (and will remain unprocessed)".format(sccs_left, nodes_left))
