...
def check_follow_imports(choice: str) -> str:
    ...
    if choice not in choices:
        raise argparse.ArgumentTypeError("invalid choice '{}' (choose from {})".format(choice, ", ".join(f"'{x}'" for x in choices)))
    return choice
    ...
    for name, section in parser.items():
        if name.startswith("mypy-"):
            ...
            if report_dirs:
                print(
                    prefix,
                    "Per-module sections should not specify reports ({})".format(", ".join(s + "_report" for s in sorted(report_dirs))),
                    file=stderr,
                )
            if set(updates) - PER_MODULE_OPTIONS:
                print(
                    prefix,
                    "Per-module sections should only specify per-module flags ({})".format(", ".join(sorted(set(updates) - PER_MODULE_OPTIONS))),
                    file=stderr,
                )
            ...
            for glob in globs.split(","):
                ...
                if any(c in glob for c in "?[]!") or any("*" in x and x != "*" for x in glob.split(".")):
                    print(
                        prefix,
                        "Patterns must be fully-qualified module names, optionally " "with '*' in some components (e.g spam.*.eggs.*)",
                        file=stderr,
                    )
                else:
...
def destructure_overrides(toml_data: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(toml_data["mypy"]["overrides"], list):
        raise ConfigTOMLValueError(
            "tool.mypy.overrides sections must be an array. Please make " "sure you are using double brackets like so: [[tool.mypy.overrides]]"
        )

    result = toml_data.copy()
    for override in result["mypy"]["overrides"]:
        if "module" not in override:
            raise ConfigTOMLValueError("toml config file contains a [[tool.mypy.overrides]] " "section, but no module to override was specified.")
        ...
        if isinstance(override["module"], str):
            ...
            raise ConfigTOMLValueError(
                "toml config file contains a [[tool.mypy.overrides]] " "section with a module value that is not a string or a list of " "strings"
            )
