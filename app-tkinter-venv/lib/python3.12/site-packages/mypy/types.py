...
class TypeVarId:
    ...
    def __eq__(self, other: object) -> bool:
        return (
            isinstance(other, TypeVarId) and self.raw_id == other.raw_id and self.meta_level == other.meta_level and self.namespace == other.namespace
        )
...
class Instance(ProperType):
    ...
    def is_singleton_type(self) -> bool:
        ...
        return self.type.is_enum and len(self.type.enum_members) == 1 or self.type.fullname in {"builtins.ellipsis", "types.EllipsisType"}
    ...
    def copy_modified(
        ...
        *,
        ...
        return Parameters(
            ...
            imprecise_arg_kinds=(imprecise_arg_kinds if imprecise_arg_kinds is not _dummy else self.imprecise_arg_kinds),
        )
...
class CallableType(FunctionLike):
    ...
    def with_unpacked_kwargs(self) -> NormalizedCallableType:
        ...
        extra_kinds = [ArgKind.ARG_NAMED if name in last_type.required_keys else ArgKind.ARG_NAMED_OPT for name in last_type.items]
...
class TupleType(ProperType):
    ...
    def slice(self, begin: int | None, end: int | None, stride: int | None, *, fallback: Instance | None) -> TupleType | None:
        if fallback is None:
            fallback = self.partial_fallback
        ...
        if any(isinstance(t, UnpackType) for t in self.items):
            ...
            if begin is None and end is None:
            ...
            elif (begin is not None and unpack_index - total < begin < 0) and (end is None or unpack_index - total < end < 0):
                ...
                if stride is None or stride == 1:
                    ...
                    return None
            elif (begin is not None and unpack_index - total < begin < 0) and (end is not None and unpack_index >= end >= 0):
...
def flatten_nested_unions(types: Sequence[Type], *, handle_type_alias_type: bool = True, handle_recursive: bool = True) -> list[Type]:
...
deserialize_map: Final = {key: obj.deserialize for key, obj in names.items() if isinstance(obj, type) and issubclass(obj, Type) and obj is not Type}
