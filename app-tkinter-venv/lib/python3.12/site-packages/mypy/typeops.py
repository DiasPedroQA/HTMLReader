...
def supported_self_type(typ: ProperType, allow_callable: bool = True, allow_instances: bool = True) -> bool:
    ...
    if allow_callable and isinstance(typ, CallableType):
        ...
        return True
    return isinstance(typ, TypeVarType) or (allow_instances and isinstance(typ, Instance) and typ != fill_typevars(typ.type))
...
def bind_self(
    ...
    if func.variables and supported_self_type(self_param_type, allow_callable=allow_callable, allow_instances=not ignore_instances):
...
def is_valid_self_type_best_effort(c: CallableType, self_type: Instance) -> bool:
    ...
    if (
        ...
        if all(
            (type(arg) is TypeVarType and type(arg.upper_bound) is Instance and arg.upper_bound.type.fullname == "builtins.object")
