...
class FuncDef(FuncItem, SymbolNode, Statement):
    ...
    def serialize(self) -> JsonDict:
        ...
        return {
            ...
            "dataclass_transform_spec": (None if self.dataclass_transform_spec is None else self.dataclass_transform_spec.serialize()),
            "deprecated": self.deprecated,
        }
    ...
    def deserialize(cls, data: JsonDict) -> FuncDef:
        ...
        ret = FuncDef(
            ...
            [],
            body,
            (None if data["type"] is None else cast(mypy.types.FunctionLike, mypy.types.deserialize_type(data["type"]))),
        )
        ...
        ret.dataclass_transform_spec = (
            DataclassTransformSpec.deserialize(data["dataclass_transform_spec"]) if data["dataclass_transform_spec"] is not None else None
        )
...
class TypeInfo(SymbolNode):
    ...
    def enum_members(self) -> list[str]:
        ...
        for name, sym in self.names.items():
            ...
            if isinstance(sym.node, Decorator):
                if any(dec.fullname == "enum.member" for dec in sym.node.decorators if isinstance(dec, RefExpr)):
                    members.append(name)
                    continue
            # Case 2:
            #
            ...
            #     class Other: ...
                ...
                if isinstance(sym.node, Var):
                    ...
                    if (isinstance(typ, mypy.types.FunctionLike) and not typ.is_type_obj()) or (  # explicit `@member` is required
                        isinstance(typ, mypy.types.Instance) and typ.type.fullname == "enum.nonmember"
                    ):
    ...
    def calculate_metaclass_type(self) -> mypy.types.Instance | None:
        ...
        candidates = [s.declared_metaclass for s in self.mro if s.declared_metaclass is not None and s.declared_metaclass.type is not None]
        for c in candidates:
    ...
    def is_metaclass(self, *, precise: bool = False) -> bool:
        return self.has_base("builtins.type") or self.fullname == "abc.ABCMeta" or (self.fallback_to_any and not precise)
    ...
    def serialize(self) -> JsonDict:
        ...
        data = {
            ...
            "dataclass_transform_spec": (self.dataclass_transform_spec.serialize() if self.dataclass_transform_spec is not None else None),
            "deprecated": self.deprecated,
        }
        return data
    ...
    def deserialize(cls, data: JsonDict) -> TypeInfo:
        ...
        ti.declared_metaclass = None if data["declared_metaclass"] is None else mypy.types.Instance.deserialize(data["declared_metaclass"])
        ti.metaclass_type = None if data["metaclass_type"] is None else mypy.types.Instance.deserialize(data["metaclass_type"])
        ...
        ti.typeddict_type = None if data["typeddict_type"] is None else mypy.types.TypedDictType.deserialize(data["typeddict_type"])
    ...
    def serialize(self, prefix: str, name: str) -> JsonDict:
        ...
        if isinstance(self.node, MypyFile):
            ...
            if prefix is not None:
                ...
                if "." in fullname and fullname != prefix + "." + name and not (isinstance(self.node, Var) and self.node.from_module_getattr):
                    assert not isinstance(self.node, PlaceholderNode), f"Definition of {fullname} is unexpectedly incomplete"
