...
def parse_test_case(case: DataDrivenTestCase) -> None:
            ...
            for arg in args:
                if arg.startswith("version"):
                    ...
                    if compare_op == ">=":
                        ...
                        if version < defaults.PYTHON3_VERSION:
                            _item_fail(f"{arg} always false since minimum runtime version is {defaults.PYTHON3_VERSION}")
                        if not 1 < len(version) < 4:
                            _item_fail(f'Only minor or patch version checks are currently supported with "==": {version_str!r}')
    ...
    for passnum in stale_modules.keys():
        ...
        if passnum in stale_modules and passnum in rechecked_modules and not stale_modules[passnum].issubset(rechecked_modules[passnum]):
