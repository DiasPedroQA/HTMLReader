    ...
    def find_modules_recursive(self, module: str) -> list[BuildSource]:
        ...
        for name in names:
            ...
            if self.options and matches_exclude(subpath, self.options.exclude, self.fscache, self.options.verbosity >= 2):
                continue
            if self.options and self.options.exclude_gitignore and matches_gitignore(subpath, self.fscache, self.options.verbosity >= 2):
                continue

            if self.fscache.isdir(subpath):
                # Only recurse into packages
                if (self.options and self.options.namespace_packages) or (
                    self.fscache.isfile(os_path_join(subpath, "__init__.py")) or self.fscache.isfile(os_path_join(subpath, "__init__.pyi"))
                ):
                    seen.add(name)
...
def verify_module(fscache: FileSystemCache, id: str, path: str, prefix: str) -> bool:
    ...
    for i in range(id.count(".")):
        path = os.path.dirname(path)
        if not any(fscache.isfile_case(os_path_join(path, f"__init__{extension}"), prefix) for extension in PYTHON_EXTENSIONS):
            return False
    return True
...
def highest_init_level(fscache: FileSystemCache, id: str, path: str, prefix: str) -> int:
    ...
    for i in range(id.count(".")):
        path = os.path.dirname(path)
        if any(fscache.isfile_case(os_path_join(path, f"__init__{extension}"), prefix) for extension in PYTHON_EXTENSIONS):
            level = i + 1
    return level
