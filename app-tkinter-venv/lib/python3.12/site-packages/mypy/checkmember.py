...
def analyze_instance_member_access(name: str, typ: Instance, mx: MemberContext, override_info: TypeInfo | None) -> Type:
    ...
    if state.find_occurrences and info.name == state.find_occurrences[0] and name == state.find_occurrences[1] and not mx.suppress_errors:
        ...
        if not mx.is_lvalue:
            for method_name in ("__getattribute__", "__getattr__"):
                ...
                if method and method.info.fullname != "builtins.object":
                    bound_method = analyze_decorator_or_funcbase_access(defn=method, itype=itype, name=method_name, mx=mx)
...
def analyze_descriptor_access(descriptor_type: Type, mx: MemberContext) -> Type:
    ...
    mx.chk.warn_deprecated_overload_item(dunder_get, mx.context, target=inferred_dunder_get_type, selftype=descriptor_type)
...
def analyze_descriptor_assign(descriptor_type: Instance, mx: MemberContext) -> Type:
    ...
    if dunder_set is None:
        mx.fail(message_registry.DESCRIPTOR_SET_NOT_CALLABLE.format(descriptor_type.str_with_options(mx.msg.options)).value)
        return AnyType(TypeOfAny.from_error)
    ...
    mx.chk.warn_deprecated_overload_item(dunder_set, mx.context, target=inferred_dunder_set_type, selftype=descriptor_type)
...
def analyze_decorator_or_funcbase_access(defn: Decorator | FuncBase, itype: Instance, name: str, mx: MemberContext) -> Type:
