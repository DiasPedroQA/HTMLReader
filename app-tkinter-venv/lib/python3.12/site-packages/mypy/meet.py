    ...
    if isinstance(declared, UnionType):
        return make_simplified_union(
            [
                ...
                # if isinstance(x, int):
                #     ...
                if (is_overlapping_types(x, narrowed, ignore_promotions=True) or is_subtype(narrowed, x, ignore_promotions=False))
            ]
        )
...
def is_enum_overlapping_union(x: ProperType, y: ProperType) -> bool:
    ...
    return (
        ...
        and any(isinstance(p := get_proper_type(z), LiteralType) and x.type == p.fallback.type for z in y.relevant_items())
    )
...
def are_typed_dicts_overlapping(left: TypedDictType, right: TypedDictType, is_overlapping: Callable[[Type, Type], bool]) -> bool:
...
def are_tuples_overlapping(left: Type, right: Type, is_overlapping: Callable[[Type, Type], bool]) -> bool:
    ...
    if is_named_instance(right.partial_fallback, "builtins.tuple") or is_named_instance(left.partial_fallback, "builtins.tuple"):
        return True
    else:
