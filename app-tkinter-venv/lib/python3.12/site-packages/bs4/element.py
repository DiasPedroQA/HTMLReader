...
_deprecated_names = dict(whitespace_re="The {name} attribute was deprecated in version 4.7.0. If you need it, make your own copy.")
#: :meta private:
            ...
            if name is True or name is None:
                ...
                for element in generator:
                    if not isinstance(element, Tag):
                        continue
                    if element.name == name or (element.name == local_name and (prefix is None or element.prefix == prefix)):
...
class NavigableString(str, PageElement):
    ...
    def __getitem__(self, key: Union[int | slice]) -> str:
        ...
        if isinstance(key, str):
            raise TypeError(
                "string indices must be integers, not '{0}'. Are you treating a NavigableString like a Tag?".format(key.__class__.__name__)
            )
...
class Tag(PageElement):
    ...
    def decode(
        ...
        for event, element in self._event_stream(iterator):
            ...
            if event is Tag.START_ELEMENT_EVENT and not string_literal_tag and not cast(Tag, element)._should_pretty_print():
    ...
    def _event_stream(self, iterator: Optional[Iterator[PageElement]] = None) -> Iterator[Tuple[_TreeTraversalEvent, PageElement]]:
