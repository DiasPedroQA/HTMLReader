...
from setuptools.command import develop, easy_install, egg_info, install, install_scripts, sdist

REQUIREMENTS_FILES = ("requirements.txt", "tools/pip-requires")
PY_REQUIREMENTS_FILES = [x % sys.version_info[0] for x in ("requirements-py%d.txt", "tools/pip-requires-py%d")]
...
class TestrTest(testr_command.Testr):
    ...
    def run(self):
        warnings.warn(
            "testr integration is deprecated in pbr 4.2 and will " "be removed in a future release. Please call your test " "runner directly",
            DeprecationWarning,
        )
...
try:
    ...
    class NoseTest(commands.nosetests):
        ...
        def run(self):
            warnings.warn(
                "nose integration in pbr is deprecated. Please use " "the native nose setuptools configuration or call " "nose directly",
                DeprecationWarning,
            )
...
ENTRY_POINTS_MAP = {"console_scripts": _script_text, "gui_scripts": _script_text, "wsgi_scripts": _wsgi_text}
...
def _get_version_from_git_target(git_dir, target_version):
    ...
    if target_version is not None and new_version > target_version:
        raise ValueError(
            "git history requires a target version of %(new)s, but target " "version is %(target)s" % dict(new=new_version, target=target_version)
        )
    if distance == 0:
        return last_semver
