...
class TestInstallWithoutPbr(base.BaseTestCase):
    ...
    def test_install_without_pbr(self):
        ...
        self.useFixture(base.CapturedSubprocess("nopbr", [python] + ["setup.py", "install"], cwd=test_pkg_dir))
        # Execute code that should only be present if the install worked.
        self.useFixture(base.CapturedSubprocess("nopbr", [python] + ["-m", "pkgReq"], cwd=test_pkg_dir))
...
class TestMarkersPip(base.BaseTestCase):
    ...
    def test_pip_versions(self):
        ...
        for module in self.modules:
            self.useFixture(base.CapturedSubprocess("pip-version", [bin_python, "-m", "pip", "install", "--upgrade", module], cwd=venv.path))
        ...
        pkgs = self._run_cmd(bin_python, ["-m", "pip", "freeze"], cwd=venv.path, allow_fail=False)[0]
...
class TestLTSSupport(base.BaseTestCase):
    ...
    def test_lts_venv_default_versions(self):
        ...
        self.useFixture(base.CapturedSubprocess("lts-support", [bin_python, "-m", "pip", "install", pbr], cwd=venv.path))
