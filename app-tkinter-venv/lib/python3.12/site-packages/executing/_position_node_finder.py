...
from .executing import EnhancedAST, NotOneValueFound, Source, assert_, function_node_types, only
...
class PositionNodeFinder(object):
    ...
    def test_for_decorator(self, node: EnhancedAST, index: int) -> None:
        if isinstance(node.parent, (ast.ClassDef, function_node_types)) and node in node.parent.decorator_list:  # type: ignore[attr-defined]
            ...
            while True:
                ...
                if not ((self.opname(index - 4) == "PRECALL" or sys.version_info >= (3, 12)) and self.opname(index) == "CALL"):  # pragma: no mutate
                    break  # pragma: no mutate
    ...
    def known_issues(self, node: EnhancedAST, instruction: dis.Instruction) -> None:
        if instruction.opname in ("COMPARE_OP", "IS_OP", "CONTAINS_OP") and isinstance(node, types_cmp_issue):
            if isinstance(node, types_cmp_issue_fix):
                ...
                comparisons = [n for n in ast.walk(node.test) if isinstance(n, ast.Compare) and len(n.ops) > 1]  # type: ignore[attr-defined]
    ...
    def verify(self, node: EnhancedAST, instruction: dis.Instruction) -> None:
        ...
        def node_match(node_type: Union[Type, Tuple[Type, ...]], **kwargs: Any) -> bool:
            ...
            return isinstance(node, node_type) and all(
                isinstance(getattr(node, k), v) if isinstance(v, type) else getattr(node, k) == v for k, v in kwargs.items()
            )
        ...
        if (
            (inst_match("LOAD_METHOD", argval="join") or inst_match("LOAD_ATTR", argval="join") or inst_match(("CALL", "BUILD_STRING")))  # 3.12
        ...
        if (node_match(ast.Name, ctx=ast.Load) or (node_match(ast.Name, ctx=ast.Store) and isinstance(node.parent, ast.AugAssign))) and inst_match(
            (
                "LOAD_NAME",
                "LOAD_FAST",
                "LOAD_FAST_CHECK",
        ...
        if sys.version_info >= (3, 12):
            ...
            if node_match(ast.TypeVarTuple) and (
                inst_match("CALL_INTRINSIC_1", argrepr="INTRINSIC_TYPEVARTUPLE") or inst_match(("STORE_FAST", "STORE_DEREF"), argrepr=node.name)
            ):
                return

            if node_match(ast.ParamSpec) and (
                inst_match("CALL_INTRINSIC_1", argrepr="INTRINSIC_PARAMSPEC") or inst_match(("STORE_FAST", "STORE_DEREF"), argrepr=node.name)
            ):
                return
