...
if sys.version_info >= (3, 11):
    ...
    class Runner:
        ...
        def run(self, coro: Coroutine[T_Retval], *, context=None) -> T_Retval:
            ...
            if threading.current_thread() is threading.main_thread() and signal.getsignal(signal.SIGINT) is signal.default_int_handler:
...
class TaskGroup(abc.TaskGroup):
                    ...
                    with CancelScope() as wait_scope:
                        while self._tasks:
                            ...
                            try:
                                ...
                                if exc_val is None or (isinstance(exc_val, CancelledError) and not is_anyio_cancellation(exc)):
                                    exc_val = exc
    ...
    def _spawn(
        ...
        if not iscoroutine(coro):
            ...
            raise TypeError(
                f"Expected {prefix}{func.__qualname__}() to return a coroutine, but " f"the return value ({coro!r}) is not a coroutine object"
            )
...
class SocketStream(abc.SocketStream):
    ...
    async def receive(self, max_bytes: int = 65536) -> bytes:
        with self._receive_guard:
            if not self._protocol.read_event.is_set() and not self._transport.is_closing() and not self._protocol.is_at_eof:
...
class UNIXSocketStream(_RawSocketMixin, abc.UNIXSocketStream):
    ...
    async def receive_fds(self, msglen: int, maxfds: int) -> tuple[bytes, list[int]]:
        ...
        with self._receive_guard:
            while True:
                try:
                    message, ancdata, flags, addr = self._raw_socket.recvmsg(msglen, socket.CMSG_LEN(maxfds * fds.itemsize))
                except BlockingIOError:
                    await self._wait_until_readable(loop)
                except OSError as exc:
                    if self._closing:
        ...
        for cmsg_level, cmsg_type, cmsg_data in ancdata:
            if cmsg_level != socket.SOL_SOCKET or cmsg_type != socket.SCM_RIGHTS:
                raise RuntimeError(
                    f"Received unexpected ancillary data; message = {message!r}, " f"cmsg_level = {cmsg_level}, cmsg_type = {cmsg_type}"
                )
