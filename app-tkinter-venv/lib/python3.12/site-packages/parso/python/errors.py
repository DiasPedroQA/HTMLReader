    ...
    if type_ == "lambdef":
        ...
        if node.children[0] == "await":
            ...
            if trailer.children[0] == "(":
                ...
                return "attribute"
    elif ("expr" in type_ and "star_expr" not in type_) or "_test" in type_ or type_ in ("term", "factor"):  # is a substring
        if version < (3, 10):
            return "operator"
        else:
            return "expression"
    ...
    def _check_assignment(self, node, is_deletion=False, is_namedexpr=False, is_aug_assign=False):
        ...
        if type_ == "lambdef":
            ...
            for child in node.children[::2]:
                self._check_assignment(child, is_deletion, is_namedexpr, is_aug_assign)
        elif "expr" in type_ and type_ != "star_expr" or "_test" in type_ or type_ in ("term", "factor"):  # is a substring
            if self._normalizer.version < (3, 10):
                error = "operator"
            else:
                error = "expression"
    ...
    def is_issue(self, node):
        ...
        if is_aug_assign:
            ...
            if target.type == "name" or (
                target.type in ("atom_expr", "power") and target.children[1].type == "trailer" and target.children[-1].children[0] != "("
            ):
                return False
