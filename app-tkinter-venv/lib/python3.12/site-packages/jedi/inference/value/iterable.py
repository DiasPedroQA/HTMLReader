...
from jedi.inference.base_value import NO_VALUES, ContextualizedNode, LazyValueWrapper, Value, ValueSet, iterate_values, sentinel
from jedi.inference.cache import inference_state_method_cache
from jedi.inference.context import CompForContext
from jedi.inference.filters import LazyAttributeOverwrite, publish_method
from jedi.inference.helpers import SimpleGetItemNotFound, get_int_or_none, is_string, reraise_getitem_errors
...
class DictComprehension(ComprehensionMixin, Sequence, _DictKeyMixin):
    ...
    def _imitate_items(self, arguments):
        lazy_values = [
            LazyKnownValue(FakeTuple(self.inference_state, [LazyKnownValues(key), LazyKnownValues(value)])) for key, value in self._iterate()
        ]
...
class DictLiteralValue(_DictMixin, SequenceLiteralValue, _DictKeyMixin):
    ...
    def _imitate_items(self, arguments):
        lazy_values = [
            LazyKnownValue(
                FakeTuple(self.inference_state, (LazyTreeValue(self._defining_context, key_node), LazyTreeValue(self._defining_context, value_node)))
            )
