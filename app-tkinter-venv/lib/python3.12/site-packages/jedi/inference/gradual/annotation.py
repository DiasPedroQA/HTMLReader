...
def _infer_annotation_string(context, string, index=None):
    ...
    if index is not None:
        value_set = value_set.filter(lambda value: (value.array_type == "tuple" and len(list(value.py__iter__())) >= index)).py__simple_getitem__(
            index
        )
    return value_set
...
def infer_return_types(function, arguments):
    ...
    return ValueSet.from_sets(
        ann.define_generics(type_var_dict) if isinstance(ann, (DefineGenericBaseClass, TypeVar)) else ValueSet({ann}) for ann in annotation_values
    ).execute_annotation()
...
def infer_return_for_callable(arguments, param_values, result_values):
    ...
    return ValueSet.from_sets(
        v.define_generics(all_type_vars) if isinstance(v, (DefineGenericBaseClass, TypeVar)) else ValueSet({v}) for v in result_values
    ).execute_annotation()
