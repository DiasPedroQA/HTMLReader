...
class Completion:
    ...
    def complete(self):
        ...
        completions = list(
            filter_names(self._inference_state, completion_names, self.stack, self._like_name, self._fuzzy, imported_names, cached_name=cached_name)
        )
        ...
        return (
            ...
            + sorted(
                completions, key=lambda x: (not x.name.startswith(self._like_name), x.name.startswith("__"), x.name.startswith("_"), x.name.lower())
            )
        )
    ...
    def _complete_python(self, leaf):
        ...
        if not kwargs_only:
            ...
            completion_names += self._complete_keywords(
                allowed_transitions, only_values=not (not current_line or current_line[-1] in " \t.;" and current_line[-3:] != "...")
            )
...
def search_in_module(
    inference_state, module_context, names, wanted_names, wanted_type, complete=False, fuzzy=False, ignore_imports=False, convert=False
):
