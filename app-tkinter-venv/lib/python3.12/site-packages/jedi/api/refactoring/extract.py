...
_VARIABLE_EXCTRACTABLE = (
    EXPRESSION_PARTS + ("atom testlist_star_expr testlist test lambdef lambdef_nocond " "keyword name number string fstring").split()
)
...
def extract_function(inference_state, path, module_context, name, pos, until_pos):
    ...
    if is_expression:
        ...
        if not has_ending_return_stmt:
            ...
            # defined variable.
            return_variables = (
                list(_find_needed_output_variables(context, nodes[0].parent, nodes[-1].end_pos, return_variables)) or [return_variables[-1]]
                if return_variables
                else []
            )
