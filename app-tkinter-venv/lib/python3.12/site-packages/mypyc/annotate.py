...
def generate_annotations(path: str, tree: MypyFile, ir: ModuleIR, type_map: dict[Expression, Type], mapper: Mapper) -> AnnotatedSource:
    anns = {}
...
class ASTAnnotateVisitor(TraverserVisitor):
    ...
    def visit_func_def(self, o: FuncDef, /) -> None:
        if self.func_depth > 0:
            self.annotate(
                o,
                "A nested function object is allocated each time statement is executed. " + "A module-level function would be faster.",
            )
        self.func_depth += 1
        ...
        for expr in o.expr:
            if isinstance(expr, CallExpr) and isinstance(expr.callee, RefExpr):
                ...
                if isinstance(node, Decorator):
                    if any(isinstance(d, RefExpr) and d.node and d.node.fullname == "contextlib.contextmanager" for d in node.decorators):
                        self.annotate(
                            expr,
    ...
    def visit_call_expr(self, o: CallExpr, /) -> None:
        ...
        if isinstance(o.callee, RefExpr) and o.callee.fullname == "builtins.isinstance" and len(o.args) == 2:
            ...
            if (class_ir and not class_ir.is_ext_class) or (class_ir is None and not info.fullname.startswith("builtins.")):
    ...
    def visit_lambda_expr(self, o: LambdaExpr, /) -> None:
        self.annotate(
            o,
            "A new object is allocated for lambda each time it is evaluated. " + "A module-level function would be faster.",
        )
