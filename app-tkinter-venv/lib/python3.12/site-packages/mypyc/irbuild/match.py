...
class MatchVisitor(TraverserVisitor):
    ...
    def visit_class_pattern(self, pattern: ClassPattern) -> None:
        ...
        isinstance_op = fast_isinstance_op if self.builder.is_builtin_ref_expr(pattern.class_ref) else slow_isinstance_op

        cond = self.builder.primitive_op(isinstance_op, [self.subject, self.builder.accept(pattern.class_ref)], pattern.line)
    ...
    def visit_mapping_pattern(self, pattern: MappingPattern) -> None:
        ...
        for key, value in zip(pattern.keys, pattern.values):
            ...
            item = self.builder.gen_method_call(self.subject, "__getitem__", [key_value], object_rprimitive, pattern.line)
    ...
    def visit_sequence_pattern(self, seq_pattern: SequencePattern) -> None:
        ...
        if capture and star_index is not None:
            ...
            capture_end = self.builder.binary_op(actual_len, self.builder.load_int(min_len - star_index), "-", capture.line)
