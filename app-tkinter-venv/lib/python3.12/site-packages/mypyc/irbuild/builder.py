...
class IRBuilder:
    ...
    def gen_method_call(
        ...
        return self.builder.gen_method_call(base, name, arg_values, result_type, line, arg_kinds, arg_names, self.can_borrow)
    ...
    def disallow_class_assignments(self, lvalues: list[Lvalue], line: int) -> None:
        ...
        for lvalue in lvalues:
            if isinstance(lvalue, MemberExpr) and isinstance(lvalue.expr, RefExpr) and isinstance(lvalue.expr.node, TypeInfo):
    ...
    def assign(self, target: Register | AssignmentTarget, rvalue_reg: Value, line: int) -> None:
        if isinstance(target, Register):
            ...
            elif (is_list_rprimitive(rvalue_reg.type) or is_tuple_rprimitive(rvalue_reg.type)) and target.star_idx is None:
    ...
    def coerce_rvalue(self, rvalue: Value, rtype: RType, line: int) -> Value:
        if is_float_rprimitive(rtype) and is_tagged(rvalue.type):
            ...
            if typename == "short_int":
                typename = "int"
            self.error(
                "Incompatible value representations in assignment " + f'(expression has type "{typename}", variable has type "float")',
                line,
            )
    ...
    def get_dict_base_type_from_type(self, target_type: Type) -> list[Instance]:
        target_type = get_proper_type(target_type)
        if isinstance(target_type, UnionType):
            return [inner for item in target_type.items for inner in self.get_dict_base_type_from_type(item)]
        ...
        if isinstance(target_type, TypedDictType):
            ...
            dict_base = next(base for base in target_type.type.mro if base.fullname == "typing.Mapping")
        elif isinstance(target_type, Instance):
    ...
    def call_refexpr_with_args(self, expr: CallExpr, callee: RefExpr, arg_values: list[Value]) -> Value:
        ...
        if callee.fullname and expr.arg_kinds == [ARG_POS] * len(arg_values):
            ...
            target = self.builder.matching_primitive_op(primitive_candidates, arg_values, expr.line, self.node_type(expr))
            if target:
                return target
        ...
        # some other reason)
        if isinstance(callee_node, Decorator) and callee_node.func not in self.fdefs_to_decorators and callee_node.func in self.singledispatch_impls:
    ...
    def load_global(self, expr: NameExpr) -> Value:
        ...
        if self.is_native_module_ref_expr(expr) and isinstance(expr.node, TypeInfo) and not self.is_synthetic_type(expr.node):
