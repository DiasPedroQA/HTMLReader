...
def translate_super_method_call(builder: IRBuilder, expr: CallExpr, callee: SuperExpr) -> Value:
    ...
    if len(callee.call.args) == 2:
        ...
        if not isinstance(typ_arg, NameExpr) or not isinstance(typ_arg.node, TypeInfo) or callee.info is not typ_arg.node:
            return translate_call(builder, expr, callee)
    ...
    for base in ir.mro[1:]:
        ...
        if ir.is_ext_class and ir.builtin_base is None and not ir.inherits_python and callee.name == "__init__" and len(expr.args) == 0:
...
def transform_comparison_expr(builder: IRBuilder, e: ComparisonExpr) -> Value:
    ...
    if first_op in ("in", "not in") and len(e.operators) == 1 and isinstance(e.operands[1], SetExpr):
        ...
        if set_literal is not None:
            lhs = e.operands[0]
            result = builder.builder.primitive_op(set_in_op, [builder.accept(lhs), set_literal], e.line, bool_rprimitive)
            if first_op == "not in":
