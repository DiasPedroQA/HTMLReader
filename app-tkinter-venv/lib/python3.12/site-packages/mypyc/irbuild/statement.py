...
def transform_block(builder: IRBuilder, block: Block) -> None:
    ...
    # those after `if MYPY`.
        ...
        builder.add(RaiseStandardError(RaiseStandardError.RUNTIME_ERROR, "Reached allegedly unreachable code!", block.line))
...
def transform_operator_assignment_stmt(builder: IRBuilder, stmt: OperatorAssignmentStmt) -> None:
    ...
    if is_tagged(builder.node_type(stmt.lvalue)) and is_tagged(builder.node_type(stmt.rvalue)) and stmt.op in int_borrow_friendly_op:
...
def transform_try_except_stmt(builder: IRBuilder, t: TryStmt) -> None:
    ...
    handlers = [(make_entry(type) if type else None, var, make_handler(body)) for type, var, body in zip(t.types, t.vars, t.handlers)]
