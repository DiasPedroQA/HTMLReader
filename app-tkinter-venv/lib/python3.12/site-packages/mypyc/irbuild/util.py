...
def is_implicit_extension_class(cdef: ClassDef) -> tuple[bool, str]:
    ...
    for d in cdef.decorators:
        if not is_trait_decorator(d) and not is_dataclass_decorator(d) and not get_mypyc_attr_call(d) and not is_final_decorator(d):
            return (
                False,
                "Classes that have decorators other than supported decorators" " can't be native classes.",
            )
    ...
    if cdef.info.metaclass_type and cdef.info.metaclass_type.type.fullname not in (
        ...
        return (
            False,
            "Classes with a metaclass other than ABCMeta, TypingMeta or" " GenericMeta can't be native classes.",
        )
    return True, ""
...
def is_constant(e: Expression) -> bool:
    ...
    return (
        ...
        or (
            ...
            and e.kind == GDEF
            and (e.fullname in ("builtins.True", "builtins.False", "builtins.None") or (isinstance(e.node, Var) and e.node.is_final))
        )
    )
