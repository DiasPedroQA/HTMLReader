...
def gen_func_body(builder: IRBuilder, sig: FuncSignature, cdef: ClassDef | None, is_singledispatch: bool) -> tuple[FuncIR, Value | None]:
...
def gen_func_ns(builder: IRBuilder) -> str:
    ...
    return "_".join(
        info.name + ("" if not info.class_name else "_" + info.class_name) for info in builder.fn_infos if info.name and info.name != "<module>"
    )
...
def get_args(builder: IRBuilder, rt_args: Sequence[RuntimeArg], line: int) -> ArgInfo:
    ...
    arg_names = [arg.name if arg.kind.is_named() or (arg.kind.is_optional() and not arg.pos_only) else None for arg in rt_args]
    ...
    for base_arg, sub_arg in zip(base_sig.real_args(), sub_sig.real_args()):
        if base_arg.type.error_overlap:
            if not base_arg.optional and sub_arg.optional and base_sig.num_bitmap_args:
                ...
                builder.error(
                    "An argument with type " + f'"{base_arg.type}" cannot be given a default value in a method override',
                    line,
                )
...
def load_type(builder: IRBuilder, typ: TypeInfo, unbounded_type: Type | None, line: int) -> Value:
    ...
    load_attr_path = (unbounded_type.name if isinstance(unbounded_type, UnboundType) else typ.fullname).removesuffix(f".{typ.name}")
    ...
    # This elif-condition finds the longest import that matches the load_attr_path.
        ...
        # any other object.
        for attr in load_attr_path.removeprefix(f"{module_name}.").split(".") if load_attr_path != module_name else []:
...
def gen_dispatch_func_ir(builder: IRBuilder, fitem: FuncDef, main_func_name: str, dispatch_name: str, sig: FuncSignature) -> tuple[FuncIR, Value]:
    ...
    func_reg = instantiate_callable_class(builder, fn_info)
    dispatch_func_ir = generate_dispatch_glue_native_function(builder, fitem, dispatch_callable_class.decl, dispatch_name)
